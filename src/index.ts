...
function scheduleVideoProcessingForRows(
	...
	for (const row of rows ?? []) {
		...
		const mediaId = typeof row.id === 'number' ? row.id : Number.parseInt(String(row.id), 10);
...
function memGet(userId: string): number | null {
	...
	if (performance.now() - entry.ts > MEM_CACHE_TTL_MS) {
		_unreadMem.delete(userId);
		return null;
	}
	return entry.count;
}
 ...
 */
async function queryStoredNews(env: Env, category: string, startUtc: string, endUtc: string, limit = 50): Promise<RssArticle[]> {
 ...
 * or a valid pubDate.
 */
async function upsertStoredNews(env: Env, category: string, articles: RssArticle[]): Promise<number> {
	const now = new Date().toISOString();
	const rows = articles
		.filter((a) => a && a.article_id && a.title && a.link && a.pubDate && !Number.isNaN(Date.parse(a.pubDate)))
		.map((a) => ({
			category,
	...
	const { error } = await sb(env).from('news_articles').upsert(rows, { onConflict: 'category,article_id' });
 ...
 */
async function queryDailyNewsSlots(env: Env, category: string, newsDate: string): Promise<NewsDailySlotRow[]> {
	const { data, error } = await sb(env)
		.from('news_daily_slots')
		.select('id, category, news_date, slot_index, source_id, article_id, title, description, image_url, link, pub_date, selected_at')
		.eq('category', category)
 ...
 */
async function insertDailyNewsSlots(env: Env, rows: NewsDailySlotRow[]): Promise<{ inserted: number; conflicts: number }> {
	let inserted = 0;
	let conflicts = 0;
	for (const row of rows) {
...
export function isSlotEligibleCandidate(a: RssArticle | null | undefined): boolean {
	return !!(a && a.title && a.title.trim() && a.link && a.link.trim() && a.pubDate && !Number.isNaN(Date.parse(a.pubDate)));
}
 ...
 */
export async function parseNewsRssItems(xmlText: string, opts: { sourceId: string; maxItems?: number }): Promise<RssArticle[]> {
 ...
 */
export function filterCandidatesToJstDay(items: RssArticle[], startUtc: string, endUtc: string): RssArticle[] {
...
async function resolveRoomMeta(
	...
	try {
		const { data } = await sb(env).from('rooms').select('icon_key, emoji').eq('id', room_id).single();
		return {
			room_id,
...
async function resolveRootPostId(
	...
	for (let hop = 0; hop < MAX_HOPS; hop++) {
		const { data: postRow } = await sb(env).from('posts').select('id, parent_post_id').eq('id', currentId).maybeSingle();
...
async function checkRoomMembership(env: Env, roomId: string, userId: string): Promise<string | null> {
	...
	const { data } = await sb(env).from('room_members').select('role').eq('room_id', roomId).eq('user_id', userId).maybeSingle();
	...
	const { data: binding } = await sb(env).from('account_devices').select('account_id').eq('device_id', userId).maybeSingle();
	...
	if (siblingRole) {
		sb(env)
			.from('room_members')
			.upsert({ room_id: roomId, user_id: userId, role: siblingRole }, { onConflict: 'room_id,user_id', ignoreDuplicates: true })
			.then(({ error }) => {
...
function parseCatalogColumns(raw: unknown): { ok: true; value: number | undefined } | { ok: false } {
	if (raw === undefined || raw === null) return { ok: true, value: undefined };
	const n =
		typeof raw === 'number' ? raw : typeof raw === 'string' && raw.trim() !== '' && Number.isFinite(Number(raw)) ? Number(raw) : NaN;
...
function parseCatalogTitleEnabled(raw: unknown): { ok: true; value: boolean | undefined } | { ok: false } {
	if (raw === undefined || raw === null) return { ok: true, value: undefined };
...
const ROOM_LINK_SUMMARY_FIELDS =
	'id,room_key,name,description,emoji,icon_key,icon_thumb_key,visibility,room_type,thread_card_style,catalog_columns,catalog_title_enabled';
...
async function loadRoomCore(env: Env, roomId: string): Promise<{ id: string; visibility: string; owner_id: string } | null> {
	const { data, error } = await sb(env).from('rooms').select('id,visibility,owner_id').eq('id', roomId).maybeSingle();
...
async function getCatalogRoomIds(env: Env, ctx: any): Promise<string[]> {
	...
	const { data, error } = await sb(env).from('rooms').select('id').eq('room_type', 'catalog');
	if (error) {
...
async function resolveAccountTeranHandle(env: Env, user_id: string): Promise<string | null> {
	const { data: binding } = await sb(env).from('account_devices').select('account_id').eq('device_id', user_id).maybeSingle();
	const accountId = (binding as any)?.account_id ?? null;
	if (!accountId) return null;
	const { data: account } = await sb(env).from('accounts').select('teran_handle').eq('id', accountId).maybeSingle();
...
export default {
	async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		const handleRequest = async (): Promise<Response> => {
			try {
				...
				if (path === '/api/identity' && req.method === 'POST') {
					...
					console.log(
						`[identity] rid=${request_id} had_cookie=${had_cookie} device_id=${device_id} origin=${req.headers.get('Origin') || 'none'} ua=${(req.headers.get('user-agent') || '').slice(0, 60)}`,
					);
				...
				if (path === '/api/posts' && req.method === 'GET') {
					...
					const isScopedQuery =
						!!post_type_param || !!root_only_param || !!mode_param || !!mood_param || !!q_param || feed_param === 'wire';
					...
					const bypassReason =
						fresh_param === '1'
							...
							: bust_param != null
								? 'bust_present'
								: after_create_post_id_param
									? 'after_create_post_id'
									: null;
					if (isFreshBypass) {
						console.log(
							`[ApiPostsMainFeedDbg]`,
					...
					const isFeed =
						...
						!room_id_param &&
						!hasPersonalizedFilters &&
						!parent_post_id_param &&
						(isStandardScopedFeed || (!post_type_param && !root_only_param && !room_scope_param && !feed_param));
					...
					if (isFeed) {
						...
						if (actor_id_param && light) {
							console.log(
								`[cache] /api/posts actor_excluded_from_key rid=${request_id} actor=${actor_id_param.slice(0, 8)} reason=light_mode`,
							);
						}
						...
						if (cached) {
							...
							console.log(
								`[perf] /api/posts cache=HIT rid=${request_id} total=${(pDone - p0).toFixed(1)}ms cache_lookup_ms=${cacheLookupMs} limit=${limit_param || 50} actor=${actor_id_param || 'none'} payloadBytes=${hitBody.length}`,
							);
						...
						cacheStatus = 'MISS';
						console.log(
							`[perf] /api/posts cache`,
							JSON.stringify({
								rid: request_id,
								cache_key: feedCacheKey?.url || 'none',
								ttl_s: FEED_CACHE_TTL,
								cache: 'MISS',
					...
					console.log(
						'[OptimizedVideoPathDebug][FeedAPI:request]',
						JSON.stringify({
							path,
							queryParams: {
								post_type: post_type_param,
								root_only: root_only_param,
								room_scope: room_scope_param,
								limit: limit_param,
								actor_id: actor_id_param ? 'set' : null,
								id: id_param || null,
								light,
								feed: feed_param || null,
							},
					...
					const feedSelectFields =
						'id,user_id,created_at,title,content,author_id,author_name,author_avatar,room_id,parent_post_id,post_type,shared_post_id,genre,mode,moods,uses_room_avatar,uses_room_display_name,room_anon_id';
					...
					const lightSelectFields =
						'id,user_id,created_at,last_activity_at,title,content,author_id,author_name,author_avatar,room_id,parent_post_id,post_type,media(type,key,thumb_key,optimized_key,processing_status,duration_ms,trim_start_ms,trim_end_ms,object_pos_x,object_pos_y),uses_room_avatar,uses_room_display_name,room_anon_id';
					...
					let q = sb(env).from('posts').select(selectFields).is('deleted_at', null);
					...
					const isWireThreadFeed =
						(feed_param === 'wire' || (post_type_param === 'thread' && room_scope_param !== 'rooms')) &&
						!id_param &&
						!user_id_param &&
						!author_id_param;
					...
					if (id_param) {
						...
						if (parent_post_id_param) {
							...
							q = q.eq('root_post_id', parsedPpid).not('parent_post_id', 'is', null);
						...
						if (q_param && q_param.trim().length > 0) {
							...
							q = q.or(`title.ilike.${keyword},content.ilike.${keyword}`);
						}
						...
						if (Number.isFinite(postId)) {
							speculativeAux = Promise.all([
								sb(env)
									.from('media')
									.select(
										'id, post_id, type, key, thumb_key, optimized_key, processing_status, width, height, duration_ms, optimized_bytes, optimized_duration_ms, trim_start_ms, trim_end_ms, object_pos_x, object_pos_y',
									)
									.eq('post_id', postId),
								sb(env).from('post_likes').select('post_id, actor_id').eq('post_id', postId),
							]);
						}
					}
					...
					if (id_param && !isReplyQuery) {
						...
						console.log(
							`[perf] /api/posts single_post_direct`,
							JSON.stringify({
								rid: request_id,
								id: parsedId,
								http_ms: httpMs,
						...
						const roomPolicyPromise: Promise<{ data: { read_policy: string } | null; error: any }> | undefined = (q as any)
							.__roomPolicyPromise;
						if (roomPolicyPromise) {
							...
							const timedPostsQ = (q as Promise<any>).then(
								(r: any) => {
									postsSubabaseMs = +(performance.now() - tParallelIssued).toFixed(1);
									return r;
								},
								(e: any) => {
									postsSubabaseMs = +(performance.now() - tParallelIssued).toFixed(1);
							...
							const timedPolicyQ = (roomPolicyPromise as Promise<any>).then(
								(r: any) => {
									policySubabaseMs = +(performance.now() - tParallelIssued).toFixed(1);
									return r;
								},
								(e: any) => {
									policySubabaseMs = +(performance.now() - tParallelIssued).toFixed(1);
							...
							console.log(
								`[perf] /api/posts room_query_breakdown`,
								JSON.stringify({
									rid: request_id,
					...
					const filterDesc = id_param
						? `id=${id_param}`
						: [
								user_id_param && `user_id=${user_id_param}`,
								author_id_param && `author_id=${author_id_param}`,
								room_id_param && `room_id=${room_id_param}`,
								post_type_param && `post_type=${post_type_param}`,
								root_only_param && `root_only=${root_only_param}`,
								parent_post_id_param && `parent_post_id=${parent_post_id_param}`,
								room_scope_param && `room_scope=${room_scope_param}`,
							]
								.filter(Boolean)
								.join(',') || 'feed';
					console.log(
						`[perf] /api/posts posts_query_split rid=${request_id} select_ms=${postsQueryMs} count_ms=0 filter=${filterDesc} limit=${limit_param || 50} rows=${posts?.length ?? 0}`,
					);
					if (postsQueryMs > 400) {
						console.log(
							`[perf] /api/posts SLOW_QUERY rid=${request_id} select_ms=${postsQueryMs} count_ms=0 filter=${filterDesc} limit=${limit_param || 50} rows=${posts?.length ?? 0}`,
						);
					}
					...
					if (isReplyQuery) {
						console.log(
							`[perf][replies] rid=${request_id} parent_post_id=${parent_post_id_param} limit=${lim} order=created_at_desc,id_desc select_ms=${postsQueryMs} http_ms=${replyHttpMs} parse_ms=${replyParseMs} rows=${posts?.length ?? 0}`,
						);
						console.log(
							`[perf][replies_http] rid=${request_id} parent_post_id=${parent_post_id_param} http_ms=${replyHttpMs} parse_ms=${replyParseMs} status=${replyStatus} server_timing="${replyHeaders.serverTiming}" x_response_time="${replyHeaders.xResponseTime}" cf_ray="${replyHeaders.cfRay}" cf_cache="${replyHeaders.cfCache}" content_range="${replyHeaders.contentRange}"`,
						);
					}
					...
					logDiag('/api/posts posts_result', {
						...
						first_id: postIds[0] ?? null,
						last_id: postIds[postIds.length - 1] ?? null,
					...
					if (postIds.length === 0) {
						...
						console.log(
							`[perf] /api/posts breakdown rid=${request_id} params=${(p1 - p0).toFixed(1)} client=${(p2 - p1).toFixed(1)} db_posts=${(p3 - p2).toFixed(1)} transform=0 total=${(p3 - p0).toFixed(1)} rows=0`,
						);
						console.log(
							`[perf] /api/posts parallel_skipped`,
							JSON.stringify({
								rid: request_id,
								reason: 'empty_posts',
								room_id: room_id_param || 'none',
								limit: limit_param || 50,
								db_posts_ms: +(p3 - p2).toFixed(1),
								total_ms: +(p3 - p0).toFixed(1),
								policy_mode: (q as any).__roomPolicyPromise ? 'parallel' : 'none',
							}),
						);
						if (isReplyQuery) {
					...
					if (id_param && postIds.length === 1 && speculativeAux) {
						...
						const profilePromise = (async () => {
							...
							profSrc = 'db';
							const tDb = Date.now();
							const { data: profRow } = await sb(env)
								.from('user_profiles')
							...
							if (profRow) {
								ctx.waitUntil(env.PROFILE_KV.put(profKvKey, JSON.stringify(profRow), { expirationTtl: 300 }).catch(() => {}));
							}
							console.log(
								`[diag] single_post_fast profile_db_fallback`,
								JSON.stringify({
									rid: request_id,
									author_id: singlePost.author_id,
									kv_miss_reason: kvMissReason,
									kv_ms: profKvMs,
									db_ms: profDbMs,
									found: !!profRow,
									wrote_kv: !!profRow,
						...
						console.log(
							`[perf] /api/posts single_post_fast`,
							JSON.stringify({
								rid: request_id,
								path: 'single_post_fast',
								select_ms: postsQueryMs,
								prof_kv_ms: profKvMs,
								prof_db_ms: profDbMs,
								prof_src: profSrc,
								aux_wait_ms: auxWaitMs,
								total_fast_ms: totalFastMs,
								total_ms: postsQueryMs + totalFastMs,
								likes: likeCount,
					...
					if (diag_param && diag_param.startsWith('only_parallel:')) {
						...
						if (which === 'media') {
							...
							const t = Date.now();
							const { data } = await sb(env).from('post_likes').select('post_id, actor_id').in('post_id', postIds);
							parallelMs = Date.now() - t;
							rowCount = (data ?? []).length;
						} else if (which === 'commentCounts') {
							const t = Date.now();
							const { data } = await sb(env).rpc('get_comment_counts', { parent_ids: postIds });
							parallelMs = Date.now() - t;
							rowCount = (data ?? []).length;
						}
						...
						logDiag(`/api/posts mode=only_parallel which=${which}`, {
							rid: request_id,
							posts_select_ms: postsSelectMs,
							[`${which}_ms`]: parallelMs,
							[`${which}_rows`]: rowCount,
						});
						logPerf('/api/posts diag_breakdown', {
							rid: request_id,
							mode: 'only_parallel',
					...
					if (light) {
						...
						const roomIds = [...new Set(lightPosts.map((p: any) => p.room_id).filter((rid: any) => rid && rid !== 'global'))] as string[];
							...
							try {
								...
								if (cachedRoomMeta) {
									...
									const { data: roomRows } = await sb(env)
										.from('rooms')
										.select(
											'id, name, visibility, room_type, icon_key, icon_thumb_key, detail_bg_image_key, detail_bg_color, detail_bg_image_opacity',
										)
										.in('id', roomIds);
									...
									ctx.waitUntil(
										edgeCache
											.put(
												roomCacheReq,
												new Response(JSON.stringify(roomMap), {
													status: 200,
													headers: {
														'Content-Type': 'application/json',
											...
											.then(() =>
												console.log(
													`[cache] room-meta put ok rid=${request_id} rooms=${roomIds.length} hash=${roomCacheHash.slice(0, 12)}`,
												),
											)
								...
								enrichedPosts = lightPosts.map((p: any) => (p.room_id && roomMap[p.room_id] ? { ...p, room: roomMap[p.room_id] } : p));
						...
						p5 = performance.now();
						console.log(
							`[perf] /api/posts light_mode rid=${request_id} posts=${enrichedPosts.length} media_rows=${totalMediaRows} media_embed=true transform_ms=${(p5 - p4).toFixed(1)} js_ms=${tJsTransform.toFixed(1)} room_db_ms=${roomDbMs.toFixed(1)} room_cache=${roomCacheStatus} room_ids=${roomIds.length} single_room=${singleRoomFeed} privacy_ms=${privacyMs} skip=likes,commentCounts,teranId,mediaSeparateFetch`,
						);
					} else {
						...
						const mediaQuery = (async () => {
							const t = Date.now();
							const { data } = await sb(env).from('media').select(mediaSelectCols).in('post_id', postIds);
							mediaMs = Date.now() - t;
							return data ?? [];
						})();
						...
						const likesQuery = (async () => {
							const t = Date.now();
							const { data } = await sb(env).from('post_likes').select(likesSelectCols).in('post_id', postIds);
							likesMs = Date.now() - t;
							return data ?? [];
						})();
						...
						const commentCountsQuery = (async () => {
							const t = Date.now();
							const { data, error: rpcErr } = await sb(env).rpc('get_comment_counts', { parent_ids: postIds });
						...
						const repostsQuery = (async () => {
							const t = Date.now();
							const { data } = await sb(env).from('posts').select('shared_post_id').in('shared_post_id', postIds).is('deleted_at', null);
						...
						try {
							...
							console.log(
								'[OptimizedVideoPathDebug][FeedAPI:liveMediaRow]',
								JSON.stringify({
									...
									media351: _row351
										? {
												id: _row351.id,
												post_id: _row351.post_id,
												key: _row351.key,
												optimized_key: _row351.optimized_key,
												thumb_key: _row351.thumb_key,
												processing_status: _row351.processing_status,
												duration_ms: _row351.duration_ms,
												optimized_duration_ms: _row351.optimized_duration_ms,
						...
						enrichedPosts = (posts ?? []).map((p: any) => {
							...
							return {
								...p,
								author_name: p.uses_room_display_name ? p.author_name : getLiveDisplayName(profile) || p.author_name,
					...
					try {
						...
						console.log(
							'[OptimizedVideoPathDebug][FeedAPI:cacheDecision]',
							JSON.stringify({
								path,
								...
								reason: !isFeed
									...
									: feedHasProcessingVideo
										? 'skip_write:unstable_video_media'
										: 'cacheable',
								rid: request_id,
								t: Date.now(),
							}),
					...
					try {
						...
						const _m351 = _p3667
							? (Array.isArray(_p3667.media) ? _p3667.media : []).find((m: any) => String(m.id) === '351') || null
							: null;
						console.log(
							'[OptimizedVideoPathDebug][FeedAPI:responsePost3667]',
							JSON.stringify({
								postPresent: !!_p3667,
								media351: _m351
									? {
											key: _m351.key,
											optimized_key: _m351.optimized_key,
											processing_status: _m351.processing_status,
											duration_ms: _m351.duration_ms,
										}
									: null,
								feedHasProcessingVideo,
					...
					try {
						...
						console.log(
							`[perf] /api/posts breakdown rid=${request_id} cache=${cacheStatus} params=${(p1 - p0).toFixed(1)} client=${(p2 - p1).toFixed(1)} db_posts=${(p3 - p2).toFixed(1)} parallel=${(p4 - p3).toFixed(1)} transform=${(p5 - p4).toFixed(1)} total=${postsTotal.toFixed(1)} rows=${enrichedPosts.length}`,
						);
						if (postsTotal >= 300) {
							console.log(
								`[perf] /api/posts breakdown2`,
								JSON.stringify({
									rid: request_id,
									filter: {
										post_type: post_type_param || 'all',
									...
									comment_counts_rows: (commentCountRows as any[]).length,
						...
						logPerf('/api/posts breakdown3', {
							...
							posts_rows: postIds.length,
							ids_count: postIds.length,
							ids_first: postIds[0] ?? null,
							ids_last: postIds[postIds.length - 1] ?? null,
							media_where_shape: mediaWhereShape,
							media_select_cols: mediaSelectCols,
							media_ms: mediaMs,
							media_rows: mediaRows.length,
							likes_where_shape: likesWhereShape,
							likes_select_cols: likesSelectCols,
							likes_ms: likesMs,
							likes_rows: (allLikeRows as any[]).length,
							commentCounts_where_shape: commentCountsWhereShape,
							commentCounts_ms: commentCountMs,
							commentCounts_rows: (commentCountRows as any[]).length,
							transform_ms: +(p5 - p4).toFixed(1),
							serialize_ms: +serializeMs.toFixed(1),
							total_ms: +postsTotal.toFixed(1),
						});
						if (isReplyQuery) {
							...
							console.log(
								`[perf][replies_breakdown] rid=${request_id} parent_post_id=${parent_post_id_param} http_ms=${replyHttpMs} parse_ms=${replyParseMs} parallel_ms=${(p4 - p3).toFixed(1)} transform_ms=${(p5 - p4).toFixed(1)} serialize_ms=${(serializeMs - p5).toFixed(1)} total_ms=${(serializeMs - p0).toFixed(1)} rows=${enrichedPosts.length} payload_bytes=${responseBody.length}`,
							);
						}
					...
					if (isFeed && feedCacheKey && !feedHasProcessingVideo) {
						ctx.waitUntil(
							cache
								...
								.then(() =>
									console.log(
										`[cache] posts/feed put ok rid=${request_id} limit=${limit_param || 50} actor=${actor_id_param || 'none'} ttl=${FEED_CACHE_TTL}s bytes=${responseBody.length}`,
									),
								)
				...
				if (path === '/api/posts' && req.method === 'POST') {
					...
					console.log(`[AUTHOR_TRACE] Stage 3: backend received`, {
						rid: request_id,
						author_name,
						author_avatar: author_avatar?.slice(0, 40),
						author_id: raw_author_id,
						user_id,
					});
					console.log(`[ROOM_FEED_DEBUG][POST_PARSE]`, {
						rid: request_id,
						room_id,
						raw_show_in_feed: body?.show_in_feed,
						rawShowInFeed,
						raw_post_type: body?.post_type,
						resolved_post_type:
							typeof body?.post_type === 'string' && ['status', 'share', 'thread'].includes(body.post_type) ? body.post_type : 'status',
					});

					// Parse reply/share fields with robust numeric coercion
					const rawParentPostId = body?.parent_post_id;
					const parent_post_id =
						rawParentPostId != null ? (Number.isFinite(Number(rawParentPostId)) ? Number(rawParentPostId) : null) : null;

					const rawPostType = body?.post_type;
					const post_type = typeof rawPostType === 'string' && ['status', 'share', 'thread'].includes(rawPostType) ? rawPostType : 'status';
					...
					if (post_type === 'thread') {
						if (title.length > LIMIT_THREAD_TITLE)
							throw new HttpError(400, 'TEXT_TOO_LONG', `Max ${LIMIT_THREAD_TITLE} characters for title`);
						if (content.length > LIMIT_THREAD_BODY)
							throw new HttpError(400, 'TEXT_TOO_LONG', `Max ${LIMIT_THREAD_BODY} characters for content`);
					} else {
					...
					const shared_post_id =
						rawSharedPostId != null ? (Number.isFinite(Number(rawSharedPostId)) ? Number(rawSharedPostId) : null) : null;
					...
					const clientRootPostId =
						rawClientRootPostId != null ? (Number.isFinite(Number(rawClientRootPostId)) ? Number(rawClientRootPostId) : null) : null;
					...
					if (!isValidMode && !isProd) {
						console.warn(`[posts][${request_id}] REJECT invalid mode`, {
							raw: rawMode,
							user_id,
					...
					if (Array.isArray(body?.moods)) {
						moods = body.moods.filter((m: unknown) => typeof m === 'string' && VALID_MOODS.has(m as string)).slice(0, MAX_MOODS);
					}
					...
					const [acctKvRaw, personaKvRaw, roomMemberKvRaw] = await Promise.all([
						needsAccountResolution ? env.PROFILE_KV.get(acctKvKey).catch(() => null) : Promise.resolve(null),
						personaKvKey ? env.PROFILE_KV.get(personaKvKey).catch(() => null) : Promise.resolve(null),
						roomMemberKvKey ? env.PROFILE_KV.get(roomMemberKvKey).catch(() => null) : Promise.resolve(null),
					]);
					...
					if (needsDbFallback) {
						step1Tasks.push(
							(async () => {
								...
								mark('acct_devices');
								accountId = deviceBinding?.account_id ? ((deviceBinding as any).account_id as string) : null;
					...
					if (needsRoomCheck && !roomMemberKvHit) {
						...
						step1Tasks.push(
							(async () => {
								...
								ctx.waitUntil(
									env.PROFILE_KV.put(roomMemberKvKey!, roomDirectRole ?? '__none__', { expirationTtl: ROOM_MEMBER_CACHE_TTL }).catch(
										() => {},
									),
								);
							})(),
					...
					const needsRoomSettings = !!room_id && room_id !== 'global' && !parent_post_id;
					...
					const personaCheckFn =
						needsPersonaCheck && !personaKvHit
							? async () => {
									...
									ctx.waitUntil(
										env.PROFILE_KV.put(`persona:dev:${user_id}:${author_id}`, '1', { expirationTtl: PERSONA_CACHE_TTL }).catch(() => {}),
									);
								}
							: needsPersonaCheck
								? async () => {
					...
					const roomFallbackFn =
						needsRoomCheck && !roomDirectRole
							? async () => {
									...
									const siblingDeviceIds = allDevices ? (allDevices as any[]).map((d: any) => d.device_id).filter(Boolean) : [];
					...
					console.log(`[ROOM_FEED_DEBUG][ROOM_ELIGIBILITY]`, {
						rid: request_id,
						room_id,
						rawShowInFeed,
						show_in_feed,
						entered_lookup: !!(rawShowInFeed && room_id && room_id !== 'global'),
					});
					...
					if (parent_post_id) {
						if (clientRootPostId && clientRootPostId > 0) {
							...
							rootPostSrc = 'db';
							console.log(`[perf] /api/posts parent_root_lookup rid=${request_id}`, {
								parent_ms: parentMs,
								root_post_id,
								src: 'db_fallback',
							});
					...
					let data: any;
					console.log(`[ROOM_FEED_DEBUG][POST_INSERT]`, {
						rid: request_id,
						room_id,
						show_in_feed,
						post_type,
					...
					try {
						...
						console.log(`[AUTHOR_TRACE] Stage 4: inserted row`, {
							rid: request_id,
							post_id: data.id,
							author_name: data.author_name,
							author_avatar: data.author_avatar?.slice(0, 40),
					...
					if (!parent_post_id && data.id) {
						...
						ctx.waitUntil(
							Promise.resolve(
								...
								console.warn(`[posts] root_post_id self-update failed`, { rid: request_id, id: data.id, error: e?.message }),
							),
						);
					}
					...
					console.log(`[perf] /api/posts insert_breakdown rid=${request_id}`, {
						feed_eligibility_ms:
							marks['feed_eligibility'] && marks['auth_done'] ? +(marks['feed_eligibility'] - marks['auth_done']).toFixed(1) : null,
						pure_insert_ms: insertMs,
						post_insert_total_ms:
							marks['post_insert_done'] && marks['auth_done'] ? +(marks['post_insert_done'] - marks['auth_done']).toFixed(1) : null,
					});
					...
					if (validatedMedia.length > 0) {
						...
						const mediaInsert = validatedMedia.map((m) => ({
							...
							processing_status: m.type === 'video' ? (m.processing_status ?? 'pending') : (m.processing_status ?? null),
						...
						if (mediaInsertError) {
							console.error(`[posts][${request_id}] media insert failed`, {
								error: mediaInsertError.message,
								postId,
								count: mediaInsert.length,
							});
						...
						ctx.waitUntil(
							(async () => {
								...
								try {
									...
									if (parentFetchError) {
										console.error(`[notif-deferred][${_notifRequestId}] failed to fetch parent post owner`, {
									...
									if (parentPost?.user_id && parentPost.user_id !== _notifUserId) {
										...
										const prResolvedRootId =
											_notifRootPostId ?? (await resolveRootPostId(env, _notifParentPostId, _notifRequestId)).rootPostId;

										console.log(
											`[CreateCommentRelatedNotification]`,
											JSON.stringify({
												type: 'post_reply',
										...
										console.warn(`[notif-deferred][${_notifRequestId}] parent post user_id not found, skipping notification`, {
											parent_post_id: _notifParentPostId,
										});
									} else {
										console.log(`[notif-deferred][${_notifRequestId}] skipping self-reply`, {
											parent_post_id: _notifParentPostId,
											user_id: _notifUserId,
										});
									}
								} catch (e: any) {
									console.error(`[notif-deferred][${_notifRequestId}] failed (non-fatal)`, {
										error: e?.message,
										parent_post_id: _notifParentPostId,
									});
								}
					...
					// that doesn't include the newly created post.
					{
						...
						const deletions = limitsToInvalidate.map((lim) => {
							...
							return cache
								.delete(new Request(cacheUrl.toString(), { method: 'GET' }))
				...
				// /api/posts/:id (DELETE) — cascade soft-delete
				{
					...
					if (m && req.method === 'DELETE') {
						...
						const { error: selfErr } = await sb(env).from('posts').update({ deleted_at: now }).eq('id', postId);
				...
				// are excluded from this route.
				{
					...
					if (m && req.method === 'PATCH') {
						...
						const { data: post, error: postErr } = await sb(env)
							.from('posts')
							.select(
								'id,user_id,room_id,post_type,parent_post_id,root_post_id,content,title,author_id,author_name,author_avatar,mode,moods,shared_post_id,created_at',
							)
							.eq('id', postId)
							.is('deleted_at', null)
							.maybeSingle();
						...
						if (postErr) {
							console.error(`[PATCH post] DB lookup error id=${postId}`, {
						...
						if (!_ownershipMatch) {
							...
							if (_callerAccountId) {
								const { data: _siblingRows } = await sb(env).from('account_devices').select('device_id').eq('account_id', _callerAccountId);
								if (_siblingRows) {
						...
						if (hasIdentityFields && !_isOriginEditor) {
							throw new HttpError(
								403,
								'FORBIDDEN',
								'Identity fields (author_name, author_avatar, title) can only be updated by teran.origin',
							);
						}
						...
						if (hasMediaField) {
							...
							if (mediaBody.keepMediaIds !== undefined) {
								...
								keepMediaIds = mediaBody.keepMediaIds.map((v: unknown) => Number(v)).filter((n: number) => Number.isFinite(n) && n > 0);
							} else {
						...
						const { data: existingMedia } = await sb(env)
							.from('media')
							.select(
								'id,type,key,thumb_key,optimized_key,processing_status,width,height,bytes,duration_ms,optimized_bytes,optimized_duration_ms,trim_start_ms,trim_end_ms,object_pos_x,object_pos_y,created_at',
							)
							.eq('post_id', postId);
						...
						if (Object.keys(postUpdate).length > 0) {
							const { error: updateErr } = await sb(env)
								...
								.eq('id', postId);
							if (updateErr) {
								console.error(`[PATCH post] canonical update error id=${postId}`, {
									error: updateErr.message,
									code: updateErr.code,
									fields: Object.keys(postUpdate),
								});
						...
						if (removedMedia.length > 0) {
							...
							const { error: mediaDelErr } = await sb(env).from('media').delete().eq('post_id', postId).in('id', removeIds);
							if (mediaDelErr) {
									...
									for (const row of removedMedia) {
										if (row.key) {
											try {
												...
												console.warn(`[PATCH post] R2 delete failed for key=`, row.key, e);
											}
										}
										if (row.thumb_key) {
											try {
												...
												console.warn(`[PATCH post] R2 delete failed for thumb_key=`, row.thumb_key, e);
											}
										}
										if (row.optimized_key) {
											try {
												await env.R2_MEDIA.delete(row.optimized_key);
											} catch (e: any) {
												console.warn(`[PATCH post] R2 delete failed for optimized_key=`, row.optimized_key, e);
											}
										}
									}
				...
				if (path === '/api/posts/appearance-bulk' && req.method === 'POST') {
					...
					const post_ids: number[] = rawIds.map((v: unknown) => Number(v)).filter((n: number) => Number.isFinite(n) && n > 0);
				...
				// PUT /api/posts/:id/appearance — create or update an override (owner-only)
				{
					...
					if (m && req.method === 'PUT') {
						...
						console.log('[Appearance403Debug]', {
							...
							rejection: !_ownershipMatch
								? 'OWNERSHIP_MISMATCH'
								: (post as any).post_type === 'share'
									? 'SHARE_POST_TYPE'
									: (post as any).parent_post_id
										? 'REPLY_POST'
										: 'none',
						});
						if (!_ownershipMatch) {
						...
						const is_enabled: boolean | undefined = typeof rawIsEnabled === 'boolean' ? rawIsEnabled : undefined;
				...
				// DELETE /api/posts/:id/appearance — remove override entirely (owner-only)
				{
					...
					if (m && req.method === 'DELETE') {
						...
						const { error: deleteErr } = await sb(env).from('post_appearance_overrides').delete().eq('post_id', postId);
				...
				// /api/comments/:id (DELETE)
				{
					...
					if (m && req.method === 'DELETE') {
						...
						const { data: mediaRows } = await sb(env).from('media').select('key, thumb_key, optimized_key').eq('comment_id', commentId);
						...
						const { error } = await sb(env).from('comments').delete().eq('id', commentId);
						if (error) throw error;
				...
				if (path === '/api/comments/counts' && req.method === 'GET') {
					...
					try {
						...
						if (cached) {
							const hitBody = await cached.text();
							console.log(
								`[perf] /api/comments/counts`,
								JSON.stringify({
									rid,
									cache: 'HIT',
									total_ms: Date.now() - t0,
									ids_count: idsCount,
									hash: cacheKeyHash.slice(0, 12),
									payloadBytes: hitBody.length,
								}),
							);
							return new Response(hitBody, {
								status: 200,
						...
						try {
							const { data: rpcRows, error: rpcErr } = await sb(env).rpc('get_comments_counts', { p_post_ids: postIds });
							if (rpcErr) {
							...
							console.log(
								`[diag] comments/counts rpc_failed`,
								JSON.stringify({
									rid,
									ids_count: idsCount,
							...
							const { data: countRows, error } = await sb(env).from('comments').select('post_id').in('post_id', postIds);
							if (error) throw error;
						...
						console.log(
							`[perf] /api/comments/counts rid=${rid} total=${Date.now() - t0}ms ids=${idsCount} error=1 msg=${err?.message?.slice(0, 100)}`,
						);
						throw err;
					}
				}
				...
				if (path === '/api/comments/preview' && req.method === 'GET') {
					...
					try {
						...
						if (cached) {
							const hitBody = await cached.text();
							console.log(
								`[perf] comments/preview rid=${rid} cache=HIT total=${Date.now() - t0}ms ids=${idsCount} per_post=${perPostUsed} hash=${cacheKeyHash.slice(0, 12)} payloadBytes=${hitBody.length}`,
							);
							return new Response(hitBody, {
								status: 200,
								headers: {
						...
						console.log(
							`[perf] comments/preview rid=${rid} cache=MISS cacheCheck=${tCache - t0}ms db=${tDb1 - tDb0}ms transform=${tEnd - tDb1}ms total=${tEnd - t0}ms payloadBytes=${body.length} ids=${idsCount} per_post=${perPostUsed} hash=${cacheKeyHash.slice(0, 12)} rows=${(rows ?? []).length} error=0`,
						);
						return new Response(body, {
							status: 200,
							headers: {
						...
						console.log(
							`[perf] comments/preview rid=${rid} total=${Date.now() - t0}ms ids=${idsCount} per_post=${perPostUsed} error=1 msg=${err?.message?.slice(0, 100)}`,
						);
						throw err;
					}
				}
				...
				if (path === '/api/posts/reply-previews' && req.method === 'GET') {
					...
					if (cached) {
						const hitBody = await cached.text();
						console.log(
							`[perf] posts/reply-previews rid=${rid} cache=HIT total=${Date.now() - t0}ms ids=${parentIds.length} hash=${cacheKeyHash.slice(0, 12)}`,
						);
						return new Response(hitBody, {
							status: 200,
							headers: {
					...
					ctx.waitUntil(
						cache
							...
							.then(() =>
								console.log(`[cache] posts/reply-previews put ok rid=${rid} ids=${parentIds.length} hash=${cacheKeyHash.slice(0, 12)}`),
							)
							.catch((err) => console.error(`[cache] posts/reply-previews put fail rid=${rid} hash=${cacheKeyHash.slice(0, 12)}`, err)),
					);

					console.log(
						`[perf] posts/reply-previews rid=${rid} cache=MISS db=${tDb1 - tDb0}ms total=${Date.now() - t0}ms ids=${parentIds.length} per_post=${perPost} hash=${cacheKeyHash.slice(0, 12)} rows=${(rows ?? []).length}`,
					);
					return new Response(body, {
						status: 200,
						headers: {
				...
				if (path === '/api/comments' && req.method === 'GET') {
					...
					const parallelQueries: PromiseLike<any>[] = [
						...
						sb(env).from('comment_likes').select('comment_id').in('comment_id', commentIds),
						// Query 3: Get liked_by_me for current user (skip if no auth)
						current_user_id
							? sb(env).from('comment_likes').select('comment_id').in('comment_id', commentIds).eq('user_id', current_user_id)
							: Promise.resolve({ data: [] }),
						// Query 4: Fetch media for all comments
						sb(env)
							.from('media')
							.select(
								'id, comment_id, type, key, thumb_key, optimized_key, processing_status, width, height, duration_ms, optimized_bytes, optimized_duration_ms, trim_start_ms, trim_end_ms, object_pos_x, object_pos_y',
							)
							.in('comment_id', commentIds),
					];
					// Query 5 (conditional): profile DB fallback only for KV misses
					if (kvMissIds.length > 0) {
						parallelQueries.push(sb(env).from('user_profiles').select('user_id, display_name, avatar').in('user_id', kvMissIds));
					}
					...
					console.log(
						...
						JSON.stringify({
							...
							select_ms: selectMs,
							prof_kv_ms: profKvMs,
							prof_kv_hits: profKvHits,
							prof_kv_misses: kvMissIds.length,
							parallel_ms: parallelMs,
							db_queries: dbQueries,
							transform_ms: transformMs,
							rows: commentList.length,
							total_ms: totalMs,
						}),
					);
				...
				if (path === '/api/comments' && req.method === 'POST') {
					...
					const content = typeof body?.content === 'string' ? body.content.trim() : '';
					const parent_comment_id = typeof body?.parent_comment_id === 'number' ? body.parent_comment_id : null;
					...
					{
						const { data: parentPost } = await sb(env).from('posts').select('room_id').eq('id', post_id).maybeSingle();
						if (!parentPost) {
					...
					if (validatedMedia.length > 0) {
						...
						const { data: insertedMedia, error: mediaError } = await sb(env).from('media').insert(mediaInsert).select('*');
					...
					if (parent_comment_id == null) {
						...
						const {
							rootPostId: pcRootPostId,
							directParentPostId: pcDirectParent,
							hops: pcHops,
						} = await resolveRootPostId(env, post_id, request_id);
						const { data: postData } = await sb(env).from('posts').select('user_id, room_id').eq('id', pcRootPostId).single();
						if (postData) {
							console.log(
								`[CreateCommentRelatedNotification]`,
								JSON.stringify({
						...
						const { data: parentComment } = await sb(env).from('comments').select('user_id, post_id').eq('id', parent_comment_id).single();
						if (parentComment) {
							// Resolve root post ID from the parent comment's post_id
							const {
								rootPostId: rpRootPostId,
								directParentPostId: rpDirectParent,
								hops: rpHops,
							} = await resolveRootPostId(env, (parentComment as any).post_id, request_id);
					...
					await sb(env).from('posts').update({ last_activity_at: new Date().toISOString() }).eq('id', post_id);
					...
					if (m) {
						...
						if (req.method === 'POST') {
							...
							const { data, error: insertError } = await sb(env)
								.from('comment_likes')
								.upsert({ comment_id, user_id }, { onConflict: 'user_id,comment_id', ignoreDuplicates: true })
								.select('comment_id');
							...
							if (wasInserted) {
								...
								if (commentData) {
									...
									const {
										rootPostId: clRootPostId,
										directParentPostId: clDirectParent,
										hops: clHops,
									} = await resolveRootPostId(env, (commentData as any).post_id, request_id);
						...
						if (req.method === 'DELETE') {
							const t1 = Date.now();
							const { error } = await sb(env).from('comment_likes').delete().eq('comment_id', comment_id).eq('user_id', user_id);
					...
					if (m) {
						...
						if (req.method === 'POST') {
							const _like = (label: string, extra?: Record<string, any>) =>
								console.log(`[perf] /api/posts/:id/like POST ${label} ${Date.now() - handlerStart}ms`, {
									post_id,
									rid: request_id,
									...extra,
								});
							...
							if (!isSelfDevice) {
								...
								if (!deviceAccountId) {
									...
									if (personaAccountId) {
										...
										console.warn(
											`[like-auth] REJECTED: unclaimed device ${jwt_user_id} tried to like as claimed actor ${actor_id} (owner account: ${personaAccountId})`,
										);
										throw new HttpError(403, 'FORBIDDEN', 'You do not own this persona');
									}
									...
									console.warn(
										`[like-auth] REJECTED: device ${jwt_user_id} account ${deviceAccountId} does not own actor ${actor_id} (persona account: ${personaAccountId})`,
									);
									throw new HttpError(403, 'FORBIDDEN', 'You do not own this persona');
								}
							} else {
							...
							const [upsertResult, ownerResult] = await Promise.all([
								sb(env)
									.from('post_likes')
									.upsert({ post_id, actor_id, actor_name, actor_avatar }, { onConflict: 'post_id,actor_id', ignoreDuplicates: true })
									.select('post_id'),
								sb(env).from('posts').select('user_id, author_id, root_post_id, room_id').eq('id', post_id).maybeSingle(),
							]);
							...
							if (wasInserted && postData?.user_id && !isSelfLike) {
								...
								_like('notif_skip_self_like');
							} else {
								_like('notif_skip_duplicate');
							}

							_like('total', { ownership_ms: ownershipMs, parallel_ms: parallelMs, wasInserted, isSelfLike });
							return ok(req, env, request_id, { liked: true }, wasInserted ? 201 : 200);
						}
						...
						if (req.method === 'DELETE') {
							...
							if (actor_id !== jwt_device_id) {
								...
								if (!deviceBinding?.account_id) {
									...
									if (claimedBinding) {
										console.warn(
											`[like-auth] REJECTED: unclaimed device ${jwt_device_id} tried to unlike as claimed actor ${actor_id} (owner account: ${(claimedBinding as any).account_id})`,
										);
									...
									if (!personaBinding) {
										console.warn(
											`[like-auth] REJECTED: device ${jwt_device_id} account ${deviceBinding.account_id} does not own actor ${actor_id}`,
										);
										throw new HttpError(403, 'FORBIDDEN', 'You do not own this like');
									}
								}
							...
							const t1 = Date.now();
							const { error } = await sb(env).from('post_likes').delete().eq('post_id', post_id).eq('actor_id', actor_id);
				...
				if (path === '/api/notifications' && req.method === 'GET') {
					...
					if (postCommentIds.length > 0) {
						const { data: commentsData } = await sb(env).from('comments').select('id, content').in('id', postCommentIds);
					...
					if (missingIds.length > 0) {
						const { data: newsCommentsData } = await sb(env).from('news_comments').select('id, content').in('id', missingIds);
					...
					if (stillMissingIds.length > 0) {
						const { data: postsData } = await sb(env).from('posts').select('id, content').in('id', stillMissingIds);
				...
				if (path === '/api/notifications/read' && req.method === 'POST') {
					...
					if (markAll) {
						...
						await sb(env).from('notifications').update({ is_read: true }).eq('recipient_user_id', user_id).eq('is_read', false);
						...
						await sb(env).from('notifications').update({ is_read: true }).eq('recipient_user_id', user_id).in('id', ids);
					} else if (group_key) {
						await sb(env).from('notifications').update({ is_read: true }).eq('recipient_user_id', user_id).eq('group_key', group_key);
					}
				...
				if (path === '/api/notifications/unread_count' && req.method === 'GET') {
						...
						ctx.waitUntil(
							(async () => {
								try {
									...
									console.log(
										`[bg_refresh] ok rid=${request_id} me=${user_id} reason=${reason} count=${r.count} http_ms=${r.httpMs} parse_ms=${r.parseMs} status=${r.status}`,
									);
								} catch (err) {
					...
					let kvResult = await Promise.race([
						...
						new Promise<typeof KV_TIMED_OUT>((resolve) => setTimeout(() => resolve(KV_TIMED_OUT), KV_TIMEOUT_1)),
					]);
					let kvAttempts = 1;
					...
					if (kvResult === KV_TIMED_OUT) {
						kvResult = await Promise.race([
							env.UNREAD_KV.get(kvKey, 'text'),
							new Promise<typeof KV_TIMED_OUT>((resolve) => setTimeout(() => resolve(KV_TIMED_OUT), KV_TIMEOUT_2)),
						]);
						kvAttempts = 2;
					}
					...
					if (cached !== null) {
						...
						console.log(
							`[perf][unread_count] rid=${request_id} me=${user_id} kv_reason=hit kv_get_ms=${kvGetMs} kv_attempts=${kvAttempts} count=${count} total_ms=${kvHitTotal.toFixed(1)} source=kv bg_refresh=started`,
						);
						if (kvHitTotal >= 200) {
							console.log(
								`[perf][unread_count] breakdown2`,
								JSON.stringify({
									rid: request_id,
									me: user_id,
									kv_reason: 'hit',
									kv_attempts: kvAttempts,
					...
					if (kvTimedOut) {
						...
						if (stale !== null) {
							...
							const pDone = performance.now();
							console.log(
								`[perf][unread_count] rid=${request_id} me=${user_id} kv_reason=timeout_stale_mem kv_get_ms=${kvGetMs} kv_attempts=${kvAttempts} count=${stale} total_ms=${(pDone - p0).toFixed(1)} source=mem bg_refresh=started`,
							);
					...
					try {
						...
						const dbTotal = p3 - p0;
						console.log(
							`[perf][unread_count] rid=${request_id} me=${user_id} kv_reason=${kvLabel} kv_get_ms=${kvGetMs} kv_attempts=${kvAttempts} count=${r.count} http_ms=${r.httpMs} parse_ms=${r.parseMs} total_ms=${dbTotal.toFixed(1)} source=db kv_put=async`,
						);
						console.log(
							`[perf][unread_count_http] rid=${request_id} me=${user_id} kv_reason=${kvLabel} http_ms=${r.httpMs} parse_ms=${r.parseMs} status=${r.status} server_timing="${r.serverTiming}" x_response_time="${r.xResponseTime}" cf_ray="${r.cfRay}" cf_cache="${r.cfCache}" content_range="${r.contentRange}"`,
						);
						if (dbTotal >= 200) {
							console.log(
								`[perf][unread_count] breakdown2`,
								JSON.stringify({
									...
									db_http_ms: +r.httpMs,
									db_parse_ms: +r.parseMs,
									kv_put_mode: 'async',
									kv_put_enqueue_ms: +kvPutEnqueueMs.toFixed(1),
				...
				if (path === '/api/debug/notifications' && req.method === 'GET') {
					...
					let query = sb(env).from('notifications').select('*').order('created_at', { ascending: false }).limit(limit);
				...
				if (path === '/api/upload-url' && req.method === 'POST') {
					...
					if (isAudio) {
						const ALLOWED_AUDIO_TYPES = new Set(['audio/webm', 'audio/mp4', 'audio/x-m4a', 'audio/aac', 'audio/mpeg', 'audio/wav']);
						const baseAudioType = content_type.split(';')[0].trim().toLowerCase();
						if (!ALLOWED_AUDIO_TYPES.has(baseAudioType)) {
							throw new HttpError(
								422,
								'VALIDATION_ERROR',
								'content_type must be a supported audio type (audio/webm, audio/mp4, audio/x-m4a, audio/aac, audio/mpeg, audio/wav) for audio kind',
							);
						}
					}
				...
				// PUT /api/upload/:key (presigned upload receiver)
				{
					...
					if (uploadMatch && req.method === 'PUT') {
						...
						console.log(
							`[upload-diag] rid=${request_id} DONE key=${key} body_bytes=${body.byteLength} body_read_ms=${bodyMs} r2_put_ms=${r2Ms} total_ms=${totalMs} colo=${cf.colo ?? '?'}`,
						);
				...
				// GET /api/media/:key - serve R2 objects
				{
					...
					if (mediaMatch) {
						...
						// ── Entry log ──
						console.log(
							`[media-diag] rid=${rid} key=${key} method=${req.method} host=${host} path=${path} range=${range} accept=${accept} cfRay=${cfRay} colo=${colo} city=${city} region=${region} country=${country} asn=${asn} httpVersion=${httpVersion}`,
						);
						...
						try {
							obj = r2Options ? await env.R2_MEDIA.get(key, r2Options) : await env.R2_MEDIA.get(key);
						} catch (r2Err) {
							const totalMs = performance.now() - t0;
							console.log(
								`[media-diag] rid=${rid} ERROR key=${key} r2_get_ms=${(performance.now() - tR2Start).toFixed(1)} total_ms=${totalMs.toFixed(1)} err=${String(r2Err)}`,
							);
							throw r2Err;
						}
						...
						if (!obj) {
							const totalMs = performance.now() - t0;
							console.log(
								`[media-diag] rid=${rid} DONE key=${key} status=404 r2_get_ms=${r2GetMs.toFixed(1)} total_ms=${totalMs.toFixed(1)} bytes_out=0 cache_control=none etag=none cfRay=${cfRay} colo=${colo}`,
							);
							return new Response(
								JSON.stringify({
						...
						const cacheControl = isThumb ? 'public, max-age=31536000, immutable' : 'public, max-age=86400';
						...
						if (rangeHeader && rangeStart !== undefined) {
							...
							const totalMs = performance.now() - t0;
							console.log(
								`[media-diag] rid=${rid} DONE key=${key} status=206 r2_get_ms=${r2GetMs.toFixed(1)} total_ms=${totalMs.toFixed(1)} bytes_out=${length} obj_size=${obj.size} range=${start}-${end} cache_control="${cacheControl}" etag=${etag} cfRay=${cfRay} colo=${colo}`,
							);
						...
						const totalMs = performance.now() - t0;
						console.log(
							`[media-diag] rid=${rid} DONE key=${key} status=200 r2_get_ms=${r2GetMs.toFixed(1)} total_ms=${totalMs.toFixed(1)} bytes_out=${obj.size} cache_control="${cacheControl}" etag=${etag} cfRay=${cfRay} colo=${colo}`,
						);
				...
				if (path === '/api/saves' && req.method === 'POST') {
					...
					const { data, error: insertError } = await sb(env)
						.from('saves')
						.upsert({ user_id, post_id }, { onConflict: 'user_id,post_id', ignoreDuplicates: true })
						.select('post_id');
					const wasInserted = (data?.length ?? 0) > 0;
				...
				// DELETE /api/saves/:post_id - Unsave a post
				{
					...
					if (m && req.method === 'DELETE') {
						...
						const { error } = await sb(env).from('saves').delete().eq('user_id', user_id).eq('post_id', post_id);
						if (error) throw error;
				...
				if (path === '/api/saves/counts' && req.method === 'GET') {
					...
					try {
						...
						if (cached) {
							const hitBody = await cached.text();
							console.log(
								`[perf] saves/counts rid=${rid} cache=HIT total=${Date.now() - t0}ms ids=${idsCount} hash=${cacheKeyHash.slice(0, 12)} payloadBytes=${hitBody.length}`,
							);
							return new Response(hitBody, {
								status: 200,
								headers: {
						...
						// Cache MISS — query DB
						const tDb0 = Date.now();
						const { data: rows, error } = await sb(env).from('saves').select('post_id').in('post_id', postIds);
						const tDb1 = Date.now();
						if (error) throw error;
						...
						console.log(
							`[perf] saves/counts rid=${rid} cache=MISS cacheCheck=${tCache - t0}ms db=${tDb1 - tDb0}ms transform=${tEnd - tDb1}ms total=${tEnd - t0}ms payloadBytes=${body.length} ids=${idsCount} hash=${cacheKeyHash.slice(0, 12)} error=0`,
						);
						return new Response(body, {
							status: 200,
							headers: {
						...
						console.log(
							`[perf] saves/counts rid=${rid} total=${Date.now() - t0}ms ids=${idsCount} error=1 msg=${err?.message?.slice(0, 100)}`,
						);
						throw err;
					}
				}
				...
				if (path === '/api/blocks' && req.method === 'POST') {
					...
					const { error } = await sb(env).from('blocks').insert({ blocker_user_id, blocked_user_id });
				...
				if (path === '/api/blocks' && req.method === 'GET') {
					...
					console.log(
						`[perf] GET /api/blocks rid=${request_id} auth=${authMs}ms db=${dbMs}ms total=${Date.now() - handlerStart}ms rows=${blocked.length}`,
					);
				...
				if (path === '/api/blocks/relations' && req.method === 'GET') {
					...
					if (cachedResponse) {
						// ─── CACHE HIT ───
						console.log(
							`[cache] blocks/relations HIT rid=${request_id} me=${my_user_id.slice(0, 8)} ids_count=${userIds.length} hash=${cacheKeyHash.slice(0, 12)}`,
						);
					...
					// ─── CACHE MISS ───
					console.log(
						`[cache] blocks/relations MISS rid=${request_id} me=${my_user_id.slice(0, 8)} ids_count=${userIds.length} hash=${cacheKeyHash.slice(0, 12)}`,
					);
					...
					const [res1, res2] = await Promise.all([
						sb(env).from('blocks').select('blocked_user_id').eq('blocker_user_id', my_user_id).in('blocked_user_id', userIds),
						sb(env).from('blocks').select('blocker_user_id').eq('blocked_user_id', my_user_id).in('blocker_user_id', userIds),
					]);
					...
					const dbMs = Date.now() - t1;
					console.log(
						`[perf] blocks/relations cache=MISS db=${dbMs}ms total=${Date.now() - handlerStart}ms rid=${request_id} me=${my_user_id.slice(0, 8)} ids_count=${userIds.length} hash=${cacheKeyHash.slice(0, 12)}`,
					);
				...
				if (path === '/api/echoes/incoming_counts' && req.method === 'GET') {
					...
					const { data: rows, error } = await sb(env).from('echoes').select('echoed_user_id').in('echoed_user_id', ids);
					if (error) throw error;
					...
					console.log(
						`[perf] /api/echoes/incoming_counts rid=${request_id} ids=${ids.length} rows=${(rows ?? []).length} t=${Date.now() - handlerStart}ms`,
					);
					return ok(req, env, request_id, { counts });
				}
				...
				if (path === '/api/accounts/me' && req.method === 'GET') {
					...
					const [{ data: account }, { data: personas }] = await Promise.all([
						sb(env).from('accounts').select('id, teran_handle, created_at').eq('id', accountId).single(),
						sb(env)
							.from('account_personas')
				...
				if (path === '/api/accounts/check-handle' && req.method === 'POST') {
					...
					if (!isValidHandle(handle)) {
						return ok(req, env, request_id, {
							available: false,
							reason:
								'Handle must be 3–30 characters, lowercase letters/numbers/periods/underscores, start and end with a letter or number.',
						});
					}

					const { data: existing } = await sb(env).from('accounts').select('id').eq('teran_handle', handle).maybeSingle();
				...
				if (path === '/api/accounts/claim' && req.method === 'POST') {
					...
					const handle = String(body?.teran_handle || '')
						.toLowerCase()
						.trim();
					...
					const rawPersonas: Array<{ author_id?: string; name?: string; avatar?: string }> = body?.personas || [];
					...
					console.log(`[AccountsClaimFix] normalized personas`, {
						rid: request_id,
						rawCount: rawPersonas.length,
						validCount: validPersonas.length,
						invalidCount,
					});
					...
					if (existingBinding) {
						...
						if (existingAccount?.teran_handle) {
							return fail(req, env, request_id, 409, 'already_claimed', 'This device is already linked to a claimed account.');
						}
					}

					// Check handle uniqueness
					const { data: handleTaken } = await sb(env).from('accounts').select('id').eq('teran_handle', handle).maybeSingle();
					...
					ctx.waitUntil(
						env.PROFILE_KV.put(`acct:dev:${user_id}`, account_id, { expirationTtl: 300 }).catch((e: any) =>
							console.warn('[claim] KV acct cache invalidation failed', { rid: request_id, error: String(e) }),
						),
					);
					...
					if (personaErr) {
						console.error(`[AccountsClaimFix] persona upsert FAILED, compensating`, {
							rid: request_id,
							error: personaErr.message,
							code: personaErr.code,
							account_id,
							personaRowCount: personaRows.length,
						});
					...
					try {
						...
						for (const p of personaRows) {
							ctx.waitUntil(
								env.PROFILE_KV.delete(`profile:${p.persona_author_id}`).catch((e: any) =>
									console.error('[claim] KV delete failed', { rid: request_id, author_id: p.persona_author_id, error: String(e) }),
								),
							);
						}
						console.log(`[AccountsClaimFix] bridged teran_id to user_profiles`, {
							rid: request_id,
							author_ids: personaRows.map((p) => p.persona_author_id),
							teran_id: handle,
						});
					} catch (bridgeErr: any) {
            ...
            `,
					...
					let allRoomMemberships = (memberships || []).filter((m: any) => m.rooms).map((m: any) => ({ ...m.rooms, my_role: m.role }));
              ...
              `,
						...
						const { data: oldEchoes } = await sb(env).from('echoes').select('echoed_user_id, created_at').in('user_id', otherDeviceIds);
					...
					console.log(`[AccountsLoginFix] login success`, {
						...
						roomsCount: allRoomMemberships.length,
						tokenIssued: true,
					});
						...
						(async () => {
							try {
								...
								await sb(env)
									...
									.upsert({ date: new Date().toISOString().slice(0, 10), user_id, is_pwa, session_s }, { onConflict: 'date,user_id' });
				...
				if (path === '/api/feed-config' && req.method === 'GET') {
					...
					try {
						...
						ctx.waitUntil(env.PROFILE_KV.put(kvKey, JSON.stringify(result), { expirationTtl: 300 }).catch(() => {}));
				...
				if (path === '/api/profile' && req.method === 'GET') {
					...
					if (kvRaw) {
						try {
							cached = JSON.parse(kvRaw);
						} catch (_) {
							kvMissReason = 'parse_error';
						}
					...
					if (cached) {
						...
						console.log(
							`[perf] /api/profile`,
							JSON.stringify({ rid: request_id, user_id, cache: 'HIT', kv_ms: +kvGetMs.toFixed(1), total_ms: +totalMs.toFixed(1), caller }),
						);
						if (totalMs >= 300) {
							console.log(
								`[perf] /api/profile breakdown2`,
								JSON.stringify({
									rid: request_id,
									user_id,
									caller,
									...
									db_select_ms: 0,
									db_transform_ms: +transformMs.toFixed(1),
									kv_put_mode: 'none',
									kv_put_ms: 0,
					...
					if (!data) {
						console.log(
							...
							JSON.stringify({
								...
								user_id,
								cache: 'MISS',
								kv_ms: +kvGetMs.toFixed(1),
								db_ms: +dbMs.toFixed(1),
								total_ms: +totalMs.toFixed(1),
								found: false,
								caller,
							}),
					...
					const tKvPut = performance.now();
					ctx.waitUntil(
						env.PROFILE_KV.put(kvKey, JSON.stringify(data), { expirationTtl: PROFILE_CACHE_TTL }).catch((e: any) =>
							console.error('[profile] KV put failed', { rid: request_id, user_id, error: String(e) }),
						),
					);
					const kvPutEnqueueMs = performance.now() - tKvPut;
					const transformMs = performance.now() - tTransform;
				...
				if (path === '/api/profile' && req.method === 'PUT') {
					...
					if (trimmedUserId !== jwt_device_id) {
						...
						if (!deviceBinding?.account_id) {
							...
							if (claimedBinding) {
								...
								console.warn(
									`[profile-auth] REJECTED: unclaimed device ${jwt_device_id} tried to update claimed persona ${trimmedUserId} (owner account: ${(claimedBinding as any).account_id})`,
								);
							...
							if (!personaBinding) {
								console.warn(
									`[profile-auth] REJECTED: device ${jwt_device_id} account ${deviceBinding.account_id} does not own persona ${trimmedUserId}`,
								);
								throw new HttpError(403, 'FORBIDDEN', 'You do not own this profile');
							}
						}
						...
						if (body.teran_id === null || body.teran_id === '') {
							...
							if (!/^[a-z0-9_]{3,20}$/.test(tid)) {
								throw new HttpError(
									422,
									'VALIDATION_ERROR',
									'teran_id must be 3-20 characters, lowercase letters, digits, and underscores only',
								);
							}
					...
					if (body?.persona_tags !== undefined) {
						...
						const cleaned = [...new Set(body.persona_tags.map((x: any) => String(x).trim()).filter(Boolean))].slice(0, 3);
					...
					console.log(
						`[profile-sync] incoming rid=${request_id} user=${trimmedUserId} dn=${incoming.display_name} avatar=${incoming.avatar?.slice(0, 30) ?? 'null'}`,
					);
					...
					const tagsMatch =
						incoming.persona_tags === undefined ||
						(current && JSON.stringify(current.persona_tags ?? []) === JSON.stringify(incoming.persona_tags));
					const teranIdMatch = incoming.teran_id === undefined || (current && (current.teran_id ?? null) === (incoming.teran_id ?? null));
					if (
						current &&
						...
						console.log(
							`[perf] profile breakdown rid=${request_id} parse=${(p1 - p0).toFixed(1)} db_read=${(p2 - p1).toFixed(1)} total=${(p2 - p0).toFixed(1)} decision=SKIP_WRITE`,
						);
					...
					ctx.waitUntil(
						env.PROFILE_KV.delete(`profile:${trimmedUserId}`).catch((e: any) =>
							console.error('[profile] KV delete failed', { rid: request_id, user_id: trimmedUserId, error: String(e) }),
						),
					);
					...
					console.log(
						`[perf] profile breakdown rid=${request_id} parse=${(p1 - p0).toFixed(1)} db_read=${(p2 - p1).toFixed(1)} db_write=${(p3 - p2).toFixed(1)} total=${(p3 - p0).toFixed(1)} decision=WRITE`,
					);
				...
				if (path === '/api/users/search' && req.method === 'GET') {
					...
					console.log(
						`[users/search] rid=${request_id} q="${q}" prefix=${(prefixData || []).length} contains=${containsData.length} total=${users.length} t_prefix=${(p1 - p0).toFixed(1)} t_contains=${(p2 - p1).toFixed(1)}`,
					);
				...
				if (path === '/api/profile-settings' && req.method === 'PUT') {
					...
					const { error } = await sb(env).from('profile_settings').upsert(settings, { onConflict: 'user_id' });

					if (error) throw error;

					console.log(
						`[profile-settings] upsert rid=${request_id} user=${user_id} posts=${settings.posts_public} threads=${settings.threads_public} rooms=${settings.rooms_public}`,
					);
				...
				if (path === '/api/profile-gallery' && req.method === 'GET') {
					...
					const { data: row, error } = await sb(env).from('profile_gallery').select('user_id, slots').eq('user_id', user_id).single();
				...
				if (path === '/api/profile-gallery' && req.method === 'PUT') {
					...
					if (galleryUserId !== jwt_device_id) {
						...
						if (!personaBinding) {
							console.warn(
								`[gallery-auth] REJECTED: device ${jwt_device_id} account ${deviceBinding.account_id} does not own persona ${galleryUserId}`,
							);
							throw new HttpError(403, 'FORBIDDEN', 'You do not own this gallery');
						}
					}
					...
					const { error } = await sb(env)
						...
						.upsert({ user_id, slots, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
					if (error) throw error;
				...
				if (path === '/api/news/comments' && req.method === 'GET') {
					...
					let commentsQuery = sb(env).from('news_comments').select('*').eq('news_id', news_id.trim());
					if (cursor) {
						commentsQuery = commentsQuery.or(
					...
					const [likesResult, userLikesResult, mediaResult] = await Promise.all([
						sb(env).from('news_comment_likes').select('comment_id').in('comment_id', commentIds),
						current_user_id
							? sb(env).from('news_comment_likes').select('comment_id').in('comment_id', commentIds).eq('user_id', current_user_id)
							: Promise.resolve({ data: [] }),
						sb(env)
							.from('media')
							.select(
								'id, news_comment_id, type, key, thumb_key, optimized_key, processing_status, width, height, bytes, duration_ms, optimized_bytes, optimized_duration_ms, trim_start_ms, trim_end_ms, object_pos_x, object_pos_y',
							)
				...
				if (path === '/api/news/comments' && req.method === 'POST') {
					...
					const parent_comment_id = body?.parent_comment_id != null ? Number(body.parent_comment_id) || null : null;
					const author_name = typeof body?.author_name === 'string' ? body.author_name : null;
					...
					const { data, error } = await sb(env).from('news_comments').insert(insertPayload).select('*').single();
					...
					if (parent_comment_id) {
						...
						try {
							...
							if (parentComment && parentComment.user_id) {
								...
								console.log(`[news-notif:create][${request_id}] reply deep-link`, {
					...
					if (m && req.method === 'DELETE') {
						...
						for (const row of mediaRows ?? []) {
							for (const k of [row.key, row.thumb_key, (row as any).optimized_key]) {
								if (!k) continue;
								try {
									await env.R2_MEDIA.delete(k);
								} catch (e: any) {
									console.warn('[DELETE news-comment] R2 delete failed', { key: k, message: e?.message });
								}
							}
						}

						// Delete the comment (cascade deletes media rows in DB)
						const { error } = await sb(env).from('news_comments').delete().eq('id', commentId);
						if (error) throw error;
				...
				// POST /api/news/comments/:id/like (toggle like)
				{
					...
					if (m && req.method === 'POST') {
						...
						const { error: insertError } = await sb(env).from('news_comment_likes').insert({ comment_id, user_id });
						...
						try {
							...
							if (commentData && commentData.user_id) {
								...
								console.log(`[news-notif:create][${request_id}] like deep-link`, {
				...
				// DELETE /api/news/comments/:id/like (unlike)
				{
					...
					if (m && req.method === 'DELETE') {
						...
						const { error } = await sb(env).from('news_comment_likes').delete().eq('comment_id', comment_id).eq('user_id', user_id);
						if (error) throw error;
				...
				if (path === '/api/news/comments/counts' && req.method === 'GET') {
					...
					try {
						...
						if (cached) {
							const hitBody = await cached.text();
							console.log(
								`[perf] news/comments/counts rid=${rid} cache=HIT total=${Date.now() - t0}ms ids=${idsCount} payloadBytes=${hitBody.length}`,
							);
							return new Response(hitBody, {
								status: 200,
								headers: {
						...
						// Cache MISS — query DB
						const tDb0 = Date.now();
						const { data: rows, error } = await sb(env).from('news_comments').select('news_id').in('news_id', newsIds);
						const tDb1 = Date.now();
						if (error) throw error;
						...
						console.log(
							`[perf] news/comments/counts rid=${rid} cache=MISS cacheCheck=${tCache - t0}ms db=${tDb1 - tDb0}ms transform=${tEnd - tDb1}ms total=${tEnd - t0}ms payloadBytes=${body.length} ids=${idsCount} error=0`,
						);
						return new Response(body, {
							status: 200,
							headers: {
						...
						console.log(
							`[perf] news/comments/counts rid=${rid} total=${Date.now() - t0}ms ids=${idsCount} error=1 msg=${err?.message?.slice(0, 100)}`,
						);
						throw err;
					}
				}
				...
				if (path === '/api/news/comments/recent' && req.method === 'GET') {
					...
					try {
						...
						if (cached) {
							const hitBody = await cached.text();
							console.log(
								`[perf] news/comments/recent rid=${rid} cache=HIT total=${Date.now() - t0}ms ids=${idsCount} limit=${limitUsed} payloadBytes=${hitBody.length}`,
							);
							return new Response(hitBody, {
								status: 200,
								headers: {
						...
						console.log(
							`[perf] news/comments/recent rid=${rid} cache=MISS cacheCheck=${tCache - t0}ms db=${tDb1 - tDb0}ms transform=${tEnd - tDb1}ms total=${tEnd - t0}ms payloadBytes=${body.length} ids=${idsCount} limit=${limitUsed} rows=${(allComments ?? []).length} error=0`,
						);
						return new Response(body, {
							status: 200,
							headers: {
						...
						console.log(
							`[perf] news/comments/recent rid=${rid} total=${Date.now() - t0}ms ids=${idsCount} limit=${limitUsed} error=1 msg=${err?.message?.slice(0, 100)}`,
						);
						throw err;
					}
				}
				...
				if (path === '/api/rss' && req.method === 'GET') {
					...
					if (cached) {
						const hitBody = await cached.text();
						console.log(
							`[perf] rss rid=${request_id} cache=HIT total=${Date.now() - t0}ms category=${category} payloadBytes=${hitBody.length} colo=${colo} key=${cacheKey.url}`,
						);
						return new Response(hitBody, {
							status: 200,
							headers: {
					...
					const respondSlots = async (results: RssArticle[], ttl: number, mode: string) => {
						...
						console.log(
							`[perf] rss rid=${request_id} cache=MISS mode=${mode} category=${category} date=${newsDate} results=${results.length} ttl=${ttl} total=${Date.now() - t0}ms colo=${colo} key=${cacheKey.url}`,
						);
						return new Response(body, {
							status: 200,
							headers: {
					...
					if (existingSlots.length >= 8) {
						return await respondSlots(existingSlots.slice(0, 8).map(slotRowToArticle), SLOT_COMPLETE_TTL, 'slots-full');
					}
					...
					if (yahooOk) {
						...
						if (yahooAssign.length > 0) {
							try {
								...
								console.log(
									`[rss] rid=${request_id} yahoo_insert category=${category} date=${newsDate} inserted=${inserted} conflicts=${conflicts}`,
								);
							} catch (e: any) {
					...
					if (canonical.length >= 8 || !secondary) {
						...
						return await respondSlots(canonical.slice(0, 8).map(slotRowToArticle), ttl, secondary ? 'slots-yahoo' : 'slots-yahoo-only');
					}
						...
						if (nhkItems.length > 0) {
							...
							if (nhkAssign.length > 0) {
								try {
									...
									console.log(
										`[rss] rid=${request_id} nhk_insert category=${category} date=${newsDate} inserted=${inserted} conflicts=${conflicts}`,
									);
								} catch (e: any) {
					...
					return await respondSlots(canonical.slice(0, 8).map(slotRowToArticle), finalTtl, 'slots-yahoo-nhk');
				}
				...
				if (path === '/api/rooms' && req.method === 'GET') {
					...
					if (owner_id_param) {
						...
						if (owner_id_param === 'me') {
							...
							if (callerAccountId) {
								const { data: siblingRows } = await sb(env).from('account_devices').select('device_id').eq('account_id', callerAccountId);
								if (siblingRows) {
					...
					console.log(
						`[perf] /api/rooms(list) breakdown`,
						JSON.stringify({
							rid: request_id,
							owner_id: owner_id_param || 'none',
							visibility_filter: owner_id_param ? 'all' : 'public',
							params_ms: +paramsMs.toFixed(1),
							db_rooms_ms: +dbRoomsMs.toFixed(1),
							db_counts_ms: +dbCountsMs.toFixed(1),
							transform_ms: +transformMs.toFixed(1),
							total_ms: +totalMs.toFixed(1),
							rows: rooms.length,
						}),
					);
				...
				if (path === '/api/rooms/mine' && req.method === 'GET') {
					...
					const { data: callerBinding } = await sb(env).from('account_devices').select('account_id').eq('device_id', user_id).maybeSingle();
            ...
            `,
				...
				// GET /api/rooms/:id — room detail
				{
					...
					if (m && req.method === 'GET') {
						...
						if (cachedRoomRes) {
							...
							cacheStatus = 'HIT';
							console.log(
								`[perf] /api/rooms/:id cache=HIT rid=${request_id} room_id=${roomId} cache_ms=${cacheLookupMs.toFixed(1)} caller=${caller}`,
							);
						} else {
							// Cache MISS — fetch from DB
							const tDbRoom = performance.now();
							const { data: roomRow, error } = await sb(env).from('rooms').select('*').eq('id', roomId).maybeSingle();
							...
							if (!roomRow) {
								...
								console.log(
									`[perf] /api/rooms/:id breakdown`,
									JSON.stringify({
										rid: request_id,
										room_id: roomId,
										cache: 'MISS',
										db_room_ms: +dbRoomMs.toFixed(1),
										db_counts_ms: 0,
										db_membership_ms: 0,
										transform_ms: 0,
										total_ms: +totalMs.toFixed(1),
						...
						if (designOnly) {
							...
							console.log(
								...
								JSON.stringify({
									...
									cache: cacheStatus,
									db_room_ms: +dbRoomMs.toFixed(1),
									cache_ms: +cacheLookupMs.toFixed(1),
									enrich_ms: 0,
									total_ms: +totalMs.toFixed(1),
									rows_room: 1,
						...
						if (fieldsParam === 'membership') {
							...
							console.log(
								...
								JSON.stringify({
									...
									db_room_ms: +dbRoomMs.toFixed(1),
									cache_ms: +cacheLookupMs.toFixed(1),
									enrich_ms: +enrichMemberMs.toFixed(1),
									total_ms: +totalMs.toFixed(1),
									rows_room: 0,
									caller,
									fast: 'membership',
						...
						const avatarOptsPromise = wantAvatarOpts
							? sb(env).from('room_avatar_options').select('*').eq('room_id', roomId).order('sort_order', { ascending: true })
							: Promise.resolve({ data: null });

						const memberRowPromise =
							uid && my_role
								? sb(env)
										.from('room_members')
						...
						const [{ data: avatarOptions }, { data: memberRow }] = await Promise.all([avatarOptsPromise, memberRowPromise]);
						...
						console.log(
							...
							JSON.stringify({
								...
								db_room_ms: +dbRoomMs.toFixed(1),
								cache_ms: +cacheLookupMs.toFixed(1),
								enrich_ms: +enrichMs.toFixed(1),
								enrich_detail: {
									auth_ms: +authMs.toFixed(1),
									membership_ms: +membershipMs.toFixed(1),
									origin_fb_ms: +originFbMs.toFixed(1),
									origin_fast_path: originFbFastPath,
								},
								parallel_ms: +parallelMs.toFixed(1),
								avatar_opts: wantAvatarOpts,
								total_ms: +totalMs.toFixed(1),
								rows_room: 1,
								avatar_opts_count: (avatarOptions ?? []).length,
								payload_bytes: payloadBytes,
								caller,
							}),
						);
				...
				if (path === '/api/rooms' && req.method === 'POST') {
					...
					const rawVisibility =
						typeof body?.visibility === 'string' && ALLOWED_VISIBILITY.has(body.visibility) ? body.visibility : 'public';
					const visibility = rawVisibility === 'private' ? 'private_invite_only' : rawVisibility;

					// ── Creator display name (optional, room identity) ──
					const rawCreatorDisplayName =
						typeof body?.creator_display_name === 'string' ? body.creator_display_name.trim().slice(0, 80) : null;
					...
					const detail_comment_text_color =
						typeof design.detailCommentTextColor === 'string' ? design.detailCommentTextColor.slice(0, 20) : null;
					const detail_accent_color = typeof design.detailAccentColor === 'string' ? design.detailAccentColor.slice(0, 20) : null;
					const detail_comment_input_bg_color =
						typeof design.detailCommentInputBgColor === 'string' ? design.detailCommentInputBgColor.slice(0, 20) : null;
					const detail_comment_input_text_color =
						typeof design.detailCommentInputTextColor === 'string' ? design.detailCommentInputTextColor.slice(0, 20) : null;
					const detail_comment_bar_bg_color =
						typeof design.detailCommentBarBgColor === 'string' ? design.detailCommentBarBgColor.slice(0, 20) : null;
					...
					const list_icon_shape =
						typeof design.listIconShape === 'string' && VALID_ICON_SHAPES.includes(design.listIconShape) ? design.listIconShape : null;
					const detail_icon_shape =
						typeof design.detailIconShape === 'string' && VALID_ICON_SHAPES.includes(design.detailIconShape)
							? design.detailIconShape
							: null;
					...
					const cardBorderVisible =
						typeof body?.card_border_visible === 'boolean' ? body.card_border_visible : design.cardBorderVisible === true;
					...
					const header_bg_image_key =
						typeof design.headerBgImageKey === 'string' && design.headerBgImageKey.length > 0 && design.headerBgImageKey.length <= 300
							? design.headerBgImageKey
							: null;
					...
					const header_height =
						typeof design.headerHeight === 'string' && VALID_HEADER_HEIGHTS.includes(design.headerHeight) ? design.headerHeight : null;
					const room_bg_image_key =
						typeof design.roomBgImageKey === 'string' && design.roomBgImageKey.length > 0 && design.roomBgImageKey.length <= 300
							? design.roomBgImageKey
							: null;
					const room_bg_image_opacity =
						room_bg_image_key &&
						typeof design.roomBgImageOpacity === 'number' &&
						design.roomBgImageOpacity >= 0 &&
						design.roomBgImageOpacity <= 1
							? design.roomBgImageOpacity
							: null;
					const card_bg_image_key =
						typeof design.cardBgImageKey === 'string' && design.cardBgImageKey.length > 0 && design.cardBgImageKey.length <= 300
							? design.cardBgImageKey
							: null;
					const card_bg_image_opacity =
						card_bg_image_key &&
						typeof design.cardBgImageOpacity === 'number' &&
						design.cardBgImageOpacity >= 0 &&
						design.cardBgImageOpacity <= 1
							? design.cardBgImageOpacity
							: null;
					...
					const card_glass_style_raw =
						typeof design.cardGlassStyle === 'string' && VALID_GLASS_STYLES.includes(design.cardGlassStyle) ? design.cardGlassStyle : null;
					...
					const header_glass_style =
						typeof design.headerGlassStyle === 'string' && VALID_GLASS_STYLES.includes(design.headerGlassStyle)
							? design.headerGlassStyle
							: null;

					// ── Step 3 detail image + glass fields ──
					const detail_bg_image_key =
						typeof design.detailBgImageKey === 'string' && design.detailBgImageKey.length > 0 && design.detailBgImageKey.length <= 300
							? design.detailBgImageKey
							: null;
					const detail_bg_image_opacity =
						detail_bg_image_key &&
						typeof design.detailBgImageOpacity === 'number' &&
						design.detailBgImageOpacity >= 0 &&
						design.detailBgImageOpacity <= 1
							? design.detailBgImageOpacity
							: null;
					const detail_card_bg_image_key =
						typeof design.detailCardBgImageKey === 'string' &&
						design.detailCardBgImageKey.length > 0 &&
						design.detailCardBgImageKey.length <= 300
							? design.detailCardBgImageKey
							: null;
					const detail_card_bg_image_opacity =
						detail_card_bg_image_key &&
						typeof design.detailCardBgImageOpacity === 'number' &&
						design.detailCardBgImageOpacity >= 0 &&
						design.detailCardBgImageOpacity <= 1
							? design.detailCardBgImageOpacity
							: null;
					const detail_card_glass_enabled = typeof design.detailCardGlassEnabled === 'boolean' ? design.detailCardGlassEnabled : null;
					const detail_card_glass_style =
						typeof design.detailCardGlassStyle === 'string' && VALID_GLASS_STYLES.includes(design.detailCardGlassStyle)
							? design.detailCardGlassStyle
							: null;
					const detail_comment_bg_image_key =
						typeof design.detailCommentBgImageKey === 'string' &&
						design.detailCommentBgImageKey.length > 0 &&
						design.detailCommentBgImageKey.length <= 300
							? design.detailCommentBgImageKey
							: null;
					const detail_comment_bg_image_opacity =
						detail_comment_bg_image_key &&
						typeof design.detailCommentBgImageOpacity === 'number' &&
						design.detailCommentBgImageOpacity >= 0 &&
						design.detailCommentBgImageOpacity <= 1
							? design.detailCommentBgImageOpacity
							: null;
					const detail_comment_glass_enabled =
						typeof design.detailCommentGlassEnabled === 'boolean' ? design.detailCommentGlassEnabled : null;
					const detail_comment_glass_style =
						typeof design.detailCommentGlassStyle === 'string' && VALID_GLASS_STYLES.includes(design.detailCommentGlassStyle)
							? design.detailCommentGlassStyle
							: null;
					const detail_comment_input_bg_image_key =
						typeof design.detailCommentInputBgImageKey === 'string' &&
						design.detailCommentInputBgImageKey.length > 0 &&
						design.detailCommentInputBgImageKey.length <= 300
							? design.detailCommentInputBgImageKey
							: null;
					const detail_comment_input_bg_image_opacity =
						detail_comment_input_bg_image_key &&
						typeof design.detailCommentInputBgImageOpacity === 'number' &&
						design.detailCommentInputBgImageOpacity >= 0 &&
						design.detailCommentInputBgImageOpacity <= 1
							? design.detailCommentInputBgImageOpacity
							: null;
					const detail_comment_input_glass_enabled =
						typeof design.detailCommentInputGlassEnabled === 'boolean' ? design.detailCommentInputGlassEnabled : null;
					const detail_comment_input_glass_style =
						typeof design.detailCommentInputGlassStyle === 'string' && VALID_GLASS_STYLES.includes(design.detailCommentInputGlassStyle)
							? design.detailCommentInputGlassStyle
							: null;
					const detail_comment_bar_bg_image_key =
						typeof design.detailCommentBarBgImageKey === 'string' &&
						design.detailCommentBarBgImageKey.length > 0 &&
						design.detailCommentBarBgImageKey.length <= 300
							? design.detailCommentBarBgImageKey
							: null;
					const detail_comment_bar_bg_image_opacity =
						detail_comment_bar_bg_image_key &&
						typeof design.detailCommentBarBgImageOpacity === 'number' &&
						design.detailCommentBarBgImageOpacity >= 0 &&
						design.detailCommentBarBgImageOpacity <= 1
							? design.detailCommentBarBgImageOpacity
							: null;
					const detail_comment_bar_glass_enabled =
						typeof design.detailCommentBarGlassEnabled === 'boolean' ? design.detailCommentBarGlassEnabled : null;
					const detail_comment_bar_glass_style =
						typeof design.detailCommentBarGlassStyle === 'string' && VALID_GLASS_STYLES.includes(design.detailCommentBarGlassStyle)
							? design.detailCommentBarGlassStyle
							: null;
					...
					const detail_reply_badge_bg_color =
						typeof design.detailReplyBadgeBgColor === 'string' ? design.detailReplyBadgeBgColor.slice(0, 20) : null;
					const detail_reply_badge_glass_enabled =
					...
					const thread_card_style =
						typeof body?.thread_card_style === 'string' && VALID_CARD_STYLES.includes(body.thread_card_style)
							? body.thread_card_style
							: null;
					const social_reply_mode =
						typeof body?.social_reply_mode === 'string' && VALID_SOCIAL_REPLY_MODES.includes(body.social_reply_mode)
							? body.social_reply_mode
							: null;
					...
					if (detail_comment_input_bg_image_opacity !== null)
						insertObj.detail_comment_input_bg_image_opacity = detail_comment_input_bg_image_opacity;
					if (detail_comment_input_glass_enabled !== null)
						insertObj.detail_comment_input_glass_enabled = detail_comment_input_glass_enabled;
					...
					if (detail_comment_bar_bg_image_opacity !== null)
						insertObj.detail_comment_bar_bg_image_opacity = detail_comment_bar_bg_image_opacity;
					...
					const { data: room, error } = await sb(env).from('rooms').insert(insertObj).select().single();
					...
					if (error) {
						console.log(
							`[perf] /api/rooms(create) error`,
							JSON.stringify({ rid: request_id, step: 'db_room_insert', code: (error as any).code, message: error.message }),
						);
						throw new Error(error.message);
					}
					...
					if (memErr) {
						...
						console.error('[room_create] membership insert failed, rolling back room', {
							rid: request_id,
					...
					console.log('[room_create] membership insert ok', {
						rid: request_id,
						room_id: (room as any).id,
						membership_ms: +dbMembershipMs.toFixed(1),
					});
					...
					if (Array.isArray(body?.avatar_options) && body.avatar_options.length > 0) {
						const rows = body.avatar_options
							.slice(0, 12)
						...
						if (rows.length > 0) {
							const { data: inserted, error: aoErr } = await sb(env).from('room_avatar_options').insert(rows).select();
							if (aoErr) {
					...
					console.log(
						`[perf] /api/rooms(create) breakdown`,
						JSON.stringify({
							rid: request_id,
							room_key,
							visibility,
							params_ms: +(tValidate - tCreateTotal).toFixed(1),
							db_insert_room_ms: +dbInsertRoomMs.toFixed(1),
					...
					if (totalMs >= 300) {
						console.log(
							`[perf] /api/rooms(create) breakdown2`,
							JSON.stringify({
								rid: request_id,
								me: user_id,
								room_key,
					...
					return ok(
						...
						env,
						request_id,
						{ room: { ...(room as any), member_count: 1 }, my_role: 'owner', avatar_options: avatarOptions },
						201,
					);
				}
				...
				// PATCH /api/rooms/:id — owner update
				{
					...
					if (m && req.method === 'PATCH') {
						...
						if (ownerRole !== 'owner') {
							...
							if ((_patchRoomBinding as any)?.account_id) {
								...
								if ((_patchRoomAccount as any)?.teran_handle === 'teran.origin') {
									...
									const { data: _patchRoomRow } = await sb(env).from('rooms').select('owner_id').eq('id', roomId).maybeSingle();
									if (_patchRoomRow) {
						...
						if (typeof design?.detailCommentInputBgColor === 'string' || typeof design?.detail_comment_input_bg_color === 'string')
							updates.detail_comment_input_bg_color = (design.detailCommentInputBgColor ?? design.detail_comment_input_bg_color).slice(
								0,
								20,
							);
						if (typeof design?.detailCommentInputTextColor === 'string' || typeof design?.detail_comment_input_text_color === 'string')
							updates.detail_comment_input_text_color = (
								design.detailCommentInputTextColor ?? design.detail_comment_input_text_color
							).slice(0, 20);
						...
						if (hasCardBorderVisible) {
							updates.card_border_visible =
								typeof body?.card_border_visible === 'boolean' ? body.card_border_visible : body?.design?.cardBorderVisible === true;
						}
						const hasCardShape =
						...
						if (typeof design?.headerBgImageKey === 'string')
							updates.header_bg_image_key =
								design.headerBgImageKey.length > 0 && design.headerBgImageKey.length <= 300 ? design.headerBgImageKey : null;
						else if (typeof design?.header_bg_image_key === 'string')
							updates.header_bg_image_key =
								design.header_bg_image_key.length > 0 && design.header_bg_image_key.length <= 300 ? design.header_bg_image_key : null;
						else if (design?.headerBgImageKey === null || design?.header_bg_image_key === null) updates.header_bg_image_key = null;
						...
						else if (design?.roomBgImageKey === null || design?.room_bg_image_key === null) updates.room_bg_image_key = null;
						// Room background image opacity (only when image key is being set)
						if (
							updates.room_bg_image_key &&
							typeof design?.roomBgImageOpacity === 'number' &&
							design.roomBgImageOpacity >= 0 &&
							design.roomBgImageOpacity <= 1
						)
							...
							updates.room_bg_image_key &&
							typeof design?.room_bg_image_opacity === 'number' &&
							design.room_bg_image_opacity >= 0 &&
							design.room_bg_image_opacity <= 1
						)
						...
						else if (design?.cardBgImageKey === null || design?.card_bg_image_key === null) updates.card_bg_image_key = null;
						...
						if (typeof design?.cardBgImageOpacity === 'number' && design.cardBgImageOpacity >= 0 && design.cardBgImageOpacity <= 1)
							...
							typeof design?.card_bg_image_opacity === 'number' &&
							design.card_bg_image_opacity >= 0 &&
							design.card_bg_image_opacity <= 1
						)
							updates.card_bg_image_opacity = design.card_bg_image_opacity;
						// Card glass mode
						if (typeof design?.cardGlassEnabled === 'boolean') updates.card_glass_enabled = design.cardGlassEnabled;
						else if (typeof design?.card_glass_enabled === 'boolean') updates.card_glass_enabled = design.card_glass_enabled;
						// Card glass style
						...
						// Header glass mode
						if (typeof design?.headerGlassEnabled === 'boolean') updates.header_glass_enabled = design.headerGlassEnabled;
						else if (typeof design?.header_glass_enabled === 'boolean') updates.header_glass_enabled = design.header_glass_enabled;
						// Header glass style
						...
						else if (design?.detailBgImageKey === null || design?.detail_bg_image_key === null) updates.detail_bg_image_key = null;
						if (typeof design?.detailBgImageOpacity === 'number' && design.detailBgImageOpacity >= 0 && design.detailBgImageOpacity <= 1)
							...
							typeof design?.detail_bg_image_opacity === 'number' &&
							design.detail_bg_image_opacity >= 0 &&
							design.detail_bg_image_opacity <= 1
						)
						...
						if (
							...
							typeof design?.detail_card_bg_image_opacity === 'number' &&
							design.detail_card_bg_image_opacity >= 0 &&
							design.detail_card_bg_image_opacity <= 1
						)
							updates.detail_card_bg_image_opacity = design.detail_card_bg_image_opacity;
						// Detail card glass
						if (typeof design?.detailCardGlassEnabled === 'boolean') updates.detail_card_glass_enabled = design.detailCardGlassEnabled;
						...
						if (
							...
							typeof design?.detail_comment_bg_image_opacity === 'number' &&
							design.detail_comment_bg_image_opacity >= 0 &&
							design.detail_comment_bg_image_opacity <= 1
						)
						...
						if (typeof design?.detailCommentGlassStyle === 'string' && VALID_GLASS_STYLES_U.includes(design.detailCommentGlassStyle))
							...
							typeof design?.detail_comment_glass_style === 'string' &&
							VALID_GLASS_STYLES_U.includes(design.detail_comment_glass_style)
						)
						...
						if (
							...
							typeof design?.detail_comment_input_bg_image_opacity === 'number' &&
							design.detail_comment_input_bg_image_opacity >= 0 &&
							design.detail_comment_input_bg_image_opacity <= 1
						)
						...
						if (
							typeof design?.detailCommentInputGlassStyle === 'string' &&
							VALID_GLASS_STYLES_U.includes(design.detailCommentInputGlassStyle)
						)
							updates.detail_comment_input_glass_style = design.detailCommentInputGlassStyle;
						else if (
							typeof design?.detail_comment_input_glass_style === 'string' &&
							VALID_GLASS_STYLES_U.includes(design.detail_comment_input_glass_style)
						)
						...
						if (
							...
							typeof design?.detail_comment_bar_bg_image_opacity === 'number' &&
							design.detail_comment_bar_bg_image_opacity >= 0 &&
							design.detail_comment_bar_bg_image_opacity <= 1
						)
						...
						if (typeof design?.detailCommentBarGlassStyle === 'string' && VALID_GLASS_STYLES_U.includes(design.detailCommentBarGlassStyle))
							...
							typeof design?.detail_comment_bar_glass_style === 'string' &&
							VALID_GLASS_STYLES_U.includes(design.detail_comment_bar_glass_style)
						)
							updates.detail_comment_bar_glass_style = design.detail_comment_bar_glass_style;
						// Detail comment interaction fields
						if (typeof design?.detailLikeVisible === 'boolean') updates.detail_like_visible = design.detailLikeVisible;
						else if (typeof design?.detail_like_visible === 'boolean') updates.detail_like_visible = design.detail_like_visible;
						if (typeof design?.detailLikeColor === 'string') updates.detail_like_color = design.detailLikeColor.slice(0, 20);
						else if (typeof design?.detail_like_color === 'string') updates.detail_like_color = design.detail_like_color.slice(0, 20);
						...
						const VALID_CARD_STYLES = ['standard', 'teran', 'social'];
						const VALID_SOCIAL_REPLY_MODES = ['x', 'reddit'];
						if (typeof body?.room_type === 'string' && VALID_ROOM_TYPES.includes(body.room_type)) updates.room_type = body.room_type;
						...
						else if (body?.social_reply_mode === null) updates.social_reply_mode = null;
				...
				// DELETE /api/rooms/:id — owner-only hard delete (room + all content)
				{
					...
					if (m && req.method === 'DELETE') {
						...
						// Verify room exists
						const { data: room, error: roomErr } = await sb(env).from('rooms').select('id').eq('id', roomId).maybeSingle();
						if (roomErr) throw roomErr;
						if (!room) throw new HttpError(404, 'NOT_FOUND', 'Room not found');

						// Step 1: Collect all post IDs in this room
						const { data: roomPosts } = await sb(env).from('posts').select('id').eq('room_id', roomId);
						const postIds = (roomPosts ?? []).map((p: any) => p.id);

						if (postIds.length > 0) {
							// Step 2: Delete notifications referencing these posts
							await sb(env).from('notifications').delete().in('post_id', postIds);

							// Step 3: Delete post_likes (no FK cascade)
							await sb(env).from('post_likes').delete().in('post_id', postIds);

							// Step 4: Delete comments on these posts
							await sb(env).from('comments').delete().in('post_id', postIds);

							// Step 5: Hard-delete posts (media rows cascade via FK ON DELETE CASCADE)
							await sb(env).from('posts').delete().eq('room_id', roomId);
						}

						// Step 6: Delete room row (room_members + room_invites cascade via FK)
						const { error: delErr } = await sb(env).from('rooms').delete().eq('id', roomId);
				...
				if (path === '/api/room-design-templates' && req.method === 'POST') {
					...
					const rawImageKeys =
						body?.image_keys && typeof body.image_keys === 'object' && !Array.isArray(body.image_keys) ? body.image_keys : {};
					...
					const visibility =
						typeof body?.visibility === 'string' && VALID_VISIBILITY.includes(body.visibility) ? body.visibility : 'private';
					...
					const { data: template, error } = await sb(env).from('room_design_templates').insert(insertObj).select().single();
					if (error) throw new Error(error.message);

					console.log(
						`[room-design-templates] create id=${(template as any).id} name=${name} step=${step} visibility=${visibility} user=${user_id}`,
					);
				...
				if (path === '/api/room-design-templates' && req.method === 'GET') {
					...
					let query = sb(env).from('room_design_templates').select('*').order('created_at', { ascending: false }).limit(100);
					...
					if (mine && uid) {
						...
						const deviceIds = [uid];
						const { data: binding } = await sb(env).from('account_devices').select('account_id').eq('device_id', uid).maybeSingle();
						if (binding?.account_id) {
							const { data: siblings } = await sb(env).from('account_devices').select('device_id').eq('account_id', binding.account_id);
							if (siblings) {
				...
				// GET /api/room-design-templates/:id — single template
				{
					...
					if (m && req.method === 'GET') {
						...
						const { data: template, error } = await sb(env).from('room_design_templates').select('*').eq('id', templateId).maybeSingle();
						if (error) throw new Error(error.message);
				...
				// DELETE /api/room-design-templates/:id — owner delete
				{
					...
					if (m && req.method === 'DELETE') {
						...
						const { error } = await sb(env).from('room_design_templates').delete().eq('id', templateId);
						if (error) throw new Error(error.message);
				...
				// POST /api/rooms/:id/join — open Room join (public or private)
				{
					...
					if (m && req.method === 'POST') {
						...
						if (!existing) {
							const { error } = await sb(env).from('room_members').insert({ room_id: roomId, user_id, role: targetRole });
							if (error) throw new Error(error.message);
							...
							const { error } = await sb(env).from('room_members').update({ role: 'member' }).eq('room_id', roomId).eq('user_id', user_id);
							if (error) throw new Error(error.message);
				...
				// POST /api/rooms/:id/avatar-choice — save user's avatar selection for a Room
				{
					...
					if (m && req.method === 'POST') {
						...
						const { error: upsertErr } = await sb(env).from('room_members').upsert(
							{
								room_id: roomId,
								user_id,
								role: 'member',
				...
				// POST /api/rooms/:id/leave — leave room
				{
					...
					if (m && req.method === 'POST') {
						...
						const { error } = await sb(env).from('room_members').delete().eq('room_id', roomId).eq('user_id', user_id);
						if (error) throw new Error(error.message);
				...
				// POST /api/rooms/:id/kick — owner-only kick
				{
					...
					if (m && req.method === 'POST') {
						...
						const { error } = await sb(env).from('room_members').delete().eq('room_id', roomId).eq('user_id', targetUserId);
						if (error) throw new Error(error.message);
				...
				// POST /api/rooms/:id/invite/new — owner generates invite token
				{
					...
					if (m && req.method === 'POST') {
						...
						await sb(env).from('room_invites').update({ revoked: true }).eq('room_id', roomId).eq('revoked', false);

						// Create new token
						const token = generateInviteToken();
						const { data, error } = await sb(env).from('room_invites').insert({ room_id: roomId, token, revoked: false }).select().single();
						if (error) throw new Error(error.message);
				...
				// POST /api/rooms/:id/invite/revoke — owner revokes invite
				{
					...
					if (m && req.method === 'POST') {
						...
						const { error } = await sb(env).from('room_invites').update({ revoked: true }).eq('room_id', roomId).eq('revoked', false);
						if (error) throw new Error(error.message);
					...
					if (m && req.method === 'POST') {
						...
						const MARKER_HDRS: Record<string, string> = {
							...
							'Access-Control-Expose-Headers':
								'X-Cache, X-Cache-Key, X-Request-Id, Cache-Control, X-Teran-Join-Handler, X-Teran-Worker-Source',
						};
				...
				// GET /api/rooms/:sourceRoomId/links — list a source Room's linked Rooms
				{
					...
					if (m && req.method === 'GET') {
						...
						const { data: targetRooms, error: roomsErr } = await sb(env).from('rooms').select(ROOM_LINK_SUMMARY_FIELDS).in('id', targetIds);
				...
				// POST /api/rooms/:sourceRoomId/links — source owner links a target Private Room
				{
					...
					if (m && req.method === 'POST') {
						...
						return ok(req, env, request_id, { link: { ...(inserted as any), room: targetSummary ?? null } }, 201);
					}
				}
				...
				// DELETE /api/rooms/:sourceRoomId/links/:linkId — source owner removes a link
				{
					...
					if (m && req.method === 'DELETE') {
						...
						const { error: delErr } = await sb(env).from('room_links').delete().eq('id', linkId).eq('source_room_id', sourceRoomId);
				...
				// ══════════════════════════════════════════════════════════════
				{
					...
					if (m && req.method === 'POST') {
						...
						const { error: memErr } = await sb(env).from('room_members').insert({ room_id: catalogRoomId, user_id, role: 'owner' });
						if (memErr) {
							console.error('[catalog_create] membership failed, rolling back room', {
								rid: request_id,
								room_id: catalogRoomId,
								error: memErr.message,
							});
						...
						const { data: inserted, error: linkErr } = await sb(env)
							...
							.single();
						if (linkErr) {
							console.error('[catalog_create] link failed, rolling back room + membership', {
								rid: request_id,
								room_id: catalogRoomId,
				...
				if (path === '/api/profile-rooms' && req.method === 'GET') {
					...
					if (rawUserId.trim() === 'me') {
						// Auth required: resolve device ID from JWT
						const deviceId = await requireAuth(req, env);

						// Resolve the account bound to this device
						const { data: deviceBinding } = await sb(env)
							.from('account_devices')
							.select('account_id')
						...
						if (!accountId) {
							throw new HttpError(403, 'TERAN_ID_REQUIRED', 'Create a Teran ID to access your profile rooms.');
						}

						// Resolve the active persona/profile for this account
						...
						const primaryPersona = primaryPersonaRaw as any;
						if (!primaryPersona?.persona_author_id) {
							throw new HttpError(403, 'PROFILE_NOT_FOUND', 'Your profile must be set up before accessing profile rooms.');
						}

						resolvedOwnerId = primaryPersona.persona_author_id;
					} else {
				...
				if (path === '/api/profile-rooms' && req.method === 'POST') {
					...
					const body = (await req.json().catch(() => null)) as any;
					const targetRoomIdRaw = typeof body?.target_room_id === 'string' ? body.target_room_id.trim() : '';
					const inviteToken = typeof body?.invite_token === 'string' ? body.invite_token.trim() : '';
					...
					// 3. Teran ID prerequisite & Active Persona Resolution
					//    Reuse exact existing identity-resolution flow from Room creation.
					const { data: deviceBinding } = await sb(env).from('account_devices').select('account_id').eq('device_id', user_id).maybeSingle();

					const accountId = (deviceBinding as any)?.account_id;
					if (!accountId) {
						throw new HttpError(403, 'TERAN_ID_REQUIRED', 'Create a Teran ID before adding rooms to your profile.');
					}
					...
					const primaryPersona = primaryPersonaRaw as any;
					if (!primaryPersona?.persona_author_id) {
						throw new HttpError(403, 'PROFILE_NOT_FOUND', 'Your profile must exist in user_profiles before adding profile rooms.');
					}

					const resolvedOwnerId = primaryPersona.persona_author_id;
					...
					const { data: profileRow } = await sb(env).from('user_profiles').select('user_id').eq('user_id', resolvedOwnerId).maybeSingle();
					if (!profileRow) {
						throw new HttpError(403, 'PROFILE_NOT_FOUND', 'Your profile must exist in user_profiles before adding profile rooms.');
					}
					...
					if (targetRoom.visibility !== 'private_invite_only') {
						throw new HttpError(400, 'TARGET_ROOM_NOT_PRIVATE', 'Only Private (invite-only) rooms may be added to a profile');
					}
					...
					const { data: inserted, error: insertErr } = await sb(env)
						.from('profile_linked_rooms')
						.insert({
							owner_user_id: resolvedOwnerId, // resolved active persona
					...
					console.log(`[profile-rooms] POST rid=${request_id} owner=${resolvedOwnerId} target=${targetRoomIdRaw} position=${nextPosition}`);
				...
				// Remove one linked Room from the authenticated owner's profile.
				{
					...
					if (m && req.method === 'DELETE') {
						...
						console.log(`[profile-rooms] DELETE rid=${request_id} owner=${resolvedOwnerId} link=${linkId}`);
		...
		try {
			...
			res = new Response(JSON.stringify({ error: { code: 'INTERNAL_ERROR', message: 'unhandled' }, request_id }), {
				status: 500,
			...
			if (is429) {
				let bodyPrefix = '';
				try {
					bodyPrefix = (await res.clone().text()).slice(0, 120);
				} catch (_) {}
				console.warn(
					`[rl] ${method} ${path} ${JSON.stringify({
						rid: request_id,
						cf_ray: cfRay,
						colo,
						status,
